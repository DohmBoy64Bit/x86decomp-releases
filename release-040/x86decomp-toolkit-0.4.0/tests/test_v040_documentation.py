...
def test_release_documents_and_maps_are_v040() -> None:
    required = [
        ROOT / "README.md",
    ...
    for path in required:
        ...
        assert "0.4.0" in text or path.name == "SECURITY.md", path
...
def test_four_architecture_artifacts_exist_and_cross_link() -> None:
    ...
    for path in (toolkit_mermaid, toolkit_ascii, suite_mermaid, suite_ascii):
        assert path.is_file()
        assert "0.4.0" in path.read_text(encoding="utf-8")
