...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("hybrid-generate"); p.add_argument("project", type=_path); p.add_argument("output", type=_path); p.add_argument("--architecture", choices=["x86", "x86_64"], default="x86"); p.add_argument("--overwrite", action="store_true")
...
def _run(args: argparse.Namespace) -> Any:
    ...
    if c == "hybrid-generate": return generate_hybrid_project(args.project, args.output, architecture=args.architecture, overwrite=args.overwrite)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = _run(args); _print(result); return 0
