...
class ProjectStateDatabase:
    def __init__(self, path: Path, *, toolkit_version: str = "0.4.0"):
...
def migrate_project(
    ...
    *,
    ...
    if current == 1:
        ...
        plan.append("1->2")
    working = _migration_2_to_3(root, working) if not dry_run else {**working, "schema_version": 3, "toolkit_release": "0.4.0"}
    plan.append("2->3")
    if dry_run:
        return {"changed": True, "from_version": current, "to_version": 3, "dry_run": True, "plan": plan, "project_preview": working}
    if backup_path is None:
        backup_path = root.parent / f"{root.name}-pre-v0.4.0-backup.tar.gz"
