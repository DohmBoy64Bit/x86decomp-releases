...
class ProjectStateDatabase:
    def __init__(self, path: Path, *, toolkit_version: str = "0.7.5"):
...
def migrate_project(
    ...
    *,
    ...
    if current == 1:
        ...
        plan.append("1->2")
    working = _migration_2_to_3(root, working) if not dry_run else {**working, "schema_version": 3, "toolkit_release": "0.7.5"}
    plan.append("2->3")
    if dry_run:
