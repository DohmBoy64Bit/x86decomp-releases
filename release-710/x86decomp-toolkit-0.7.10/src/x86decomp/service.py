...
def _json_files(directory: Path) -> list[dict[str, Any]]:
    """Support json files processing for internal toolkit callers."""
...
def create_app(project_root: Path) -> Any:
    """Create app for the current toolkit workflow."""
    try:
    ...
    def health() -> dict[str, Any]:
        """Execute the health operation for the current toolkit workflow."""
    ...
    def project() -> dict[str, Any]:
        """Execute the project operation for the current toolkit workflow."""
        return service_snapshot(root)

    @app.get("/api/target-pack")
    def target_pack() -> dict[str, Any]:
        """Execute the target pack operation for the current toolkit workflow."""
    ...
    def workflow(function_id: str) -> dict[str, Any]:
        """Execute the workflow operation for the current toolkit workflow."""
    ...
    def next_task(mode: str | None = None) -> dict[str, Any] | None:
        """Execute the next task operation for the current toolkit workflow."""
    ...
    def reports() -> list[str]:
        """Execute the reports operation for the current toolkit workflow."""
...
def run_service(project_root: Path, *, host: str = "127.0.0.1", port: int = 8765) -> None:
    """Run service for the current toolkit workflow."""
    try:
        import uvicorn
    except ImportError as exc:
