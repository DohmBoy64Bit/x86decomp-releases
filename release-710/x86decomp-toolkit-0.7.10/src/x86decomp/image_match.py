...
def _pe_offsets(data: bytes) -> dict[str, int]:
    """Support pe offsets processing for internal toolkit callers."""
...
def derive_layout_profile(reference: Path, *, output: Path | None = None) -> dict[str, Any]:
    """Execute the derive layout profile operation for the current toolkit workflow."""
...
def _mask_ranges(data: bytes, ranges: Iterable[ByteRange]) -> bytes:
    """Support mask ranges processing for internal toolkit callers."""
...
def _rva_to_file_offset(image: Any, rva: int, file_size: int) -> int | None:
    """Support rva to file offset processing for internal toolkit callers."""
...
def _profile_ranges(data: bytes, profile: dict[str, Any], image: Any) -> list[ByteRange]:
    """Support profile ranges processing for internal toolkit callers."""
...
def _apply_rebase(data: bytes, *, target_base: int, candidate_base: int, image: Any) -> bytes:
    """Support apply rebase processing for internal toolkit callers."""
...
def _mismatches(left: bytes, right: bytes, limit: int = 256) -> tuple[int, list[dict[str, int]]]:
    """Support mismatches processing for internal toolkit callers."""
    count = 0
...
def compare_whole_images(
    ...
    *,
    ...
    """Compare whole images for the current toolkit workflow."""
