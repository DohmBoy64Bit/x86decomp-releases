...
class JobState(StrEnum):
    """Coordinate job state behavior for the current toolkit workflow."""
    PENDING = "pending"
    RUNNING = "running"
    SUCCEEDED = "succeeded"
    FAILED = "failed"
...
class PipelineStage:
    ...
    def from_dict(cls, value: Any) -> "PipelineStage":
        ...
        def strings(key: str) -> tuple[str, ...]:
            """Execute the strings operation for the current toolkit workflow."""
            raw = value.get(key, [])
...
class Orchestrator:
    """Coordinate orchestrator behavior for the current toolkit workflow."""
    ...
    def _transaction(self) -> None:
        """Support transaction processing for internal toolkit callers."""
    ...
    def _idempotency_key(self, stage: PipelineStage) -> str:
        """Support idempotency key processing for internal toolkit callers."""
    ...
    def _event(self, job_id: str, event: str, details: dict[str, Any]) -> None:
        """Support event processing for internal toolkit callers."""
        self.connection.execute(
    ...
    def _materialize_outputs(
        ...
        """Support materialize outputs processing for internal toolkit callers."""
    ...
    def _dependency_states(self, pipeline_id: str, dependencies: Iterable[str]) -> dict[str, str]:
        """Support dependency states processing for internal toolkit callers."""
        result: dict[str, str] = {}
    ...
    def _set_state(self, job_id: str, state: JobState, *, result: dict[str, Any] | None = None, error: str | None = None) -> None:
        """Support set state processing for internal toolkit callers."""
        now = utc_now()
    ...
    def _run_stage(self, manifest: PipelineManifest, stage: PipelineStage, job_id: str) -> dict[str, Any]:
        """Support run stage processing for internal toolkit callers."""
        now = utc_now()
        ...
        def cancellation_requested() -> bool:
            """Execute the cancellation requested operation for the current toolkit workflow."""
    ...
    def cancel(self, pipeline_id: str, stage_id: str | None = None) -> dict[str, Any]:
        """Execute the cancel operation for the current toolkit workflow."""
        self._transaction()
        try:
            if stage_id is None:
                cursor = self.connection.execute(
    ...
    def retry(self, pipeline_id: str, stage_id: str, *, cascade: bool = False) -> dict[str, Any]:
        """Execute the retry operation for the current toolkit workflow."""
        row = self.connection.execute(
    ...
    def _update_pipeline_status(self, pipeline_id: str) -> None:
        """Support update pipeline status processing for internal toolkit callers."""
    ...
    def status(self, pipeline_id: str) -> dict[str, Any]:
        """Execute the status operation for the current toolkit workflow."""
