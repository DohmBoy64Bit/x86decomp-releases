...
def _decimal(field: bytes, context: str, *, optional: bool = True) -> int | None:
    """Support decimal processing for internal toolkit callers."""
...
def _octal(field: bytes, context: str) -> int | None:
    """Support octal processing for internal toolkit callers."""
    text = field.decode("ascii", errors="strict").strip()
    if not text:
        return None
    try:
...
def _long_name(table: bytes, offset: int) -> str:
    """Support long name processing for internal toolkit callers."""
...
def _parse_import_object(data: bytes) -> dict[str, Any] | None:
    """Support parse import object processing for internal toolkit callers."""
    if len(data) < 20:
        return None
...
def _cstring_sequence(data: bytes, offset: int, count: int) -> tuple[list[str], int]:
    """Support cstring sequence processing for internal toolkit callers."""
    names: list[str] = []
    cursor = offset
...
def _first_linker_symbols(data: bytes) -> list[tuple[str, int]] | None:
    """Support first linker symbols processing for internal toolkit callers."""
    if len(data) < 4:
        return None
...
def _second_linker_symbols(data: bytes) -> list[tuple[str, int]] | None:
    """Support second linker symbols processing for internal toolkit callers."""
    if len(data) < 8:
        return None
...
def parse_coff_archive_bytes(data: bytes, *, path: Path | None = None) -> CoffArchive:
    """Parse coff archive bytes for the current toolkit workflow."""
...
def parse_coff_archive(path: Path) -> CoffArchive:
    """Parse coff archive for the current toolkit workflow."""
