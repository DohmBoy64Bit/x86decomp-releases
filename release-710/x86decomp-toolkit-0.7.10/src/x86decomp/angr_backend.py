...
def _load_angr() -> tuple[Any, Any]:
    """Support load angr processing for internal toolkit callers."""
    try:
...
def _arch_name(architecture: str) -> str:
    """Support arch name processing for internal toolkit callers."""
    if architecture == "x86":
        return "x86"
...
def _summaries(
    ...
    *,
    ...
    """Support summaries processing for internal toolkit callers."""
...
def _counterexample(left: list[dict[str, Any]], right: list[dict[str, Any]], output_registers: tuple[str, ...]) -> dict[str, int] | None:
    """Support counterexample processing for internal toolkit callers."""
...
def angr_bounded_compare(
    ...
    *,
    ...
    """Execute the angr bounded compare operation for the current toolkit workflow."""
...
def angr_bounded_compare_files(target: Path, candidate: Path, **kwargs: Any) -> dict[str, Any]:
    """Execute the angr bounded compare files operation for the current toolkit workflow."""
    left = target.resolve()
...
def _load_memory_harness(path: Path) -> dict[str, Any]:
    """Support load memory harness processing for internal toolkit callers."""
...
def _memory_summaries(
    ...
    *,
    ...
    """Support memory summaries processing for internal toolkit callers."""
    angr, claripy = _load_angr()
...
def _memory_counterexample(
    ...
    """Support memory counterexample processing for internal toolkit callers."""
...
def angr_memory_alias_compare(
    ...
    *,
    ...
    """Execute the angr memory alias compare operation for the current toolkit workflow."""
...
def angr_memory_alias_compare_files(
    ...
    *,
    ...
    """Execute the angr memory alias compare files operation for the current toolkit workflow."""
