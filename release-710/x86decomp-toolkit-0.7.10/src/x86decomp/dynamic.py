...
def _unicorn() -> tuple[Any, Any, Any]:
    """Support unicorn processing for internal toolkit callers."""
    try:
...
def _align_down(value: int) -> int:
    """Support align down processing for internal toolkit callers."""
    return value & ~(PAGE_SIZE - 1)


def _align_up(value: int) -> int:
    """Support align up processing for internal toolkit callers."""
    return (value + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1)


@dataclass(frozen=True)
class MemoryRegion:
    """Store validated memory region fields used by toolkit reports and adapters."""
    address: int
    data: bytes
    size: int
...
@dataclass(frozen=True)
class ExecutionSpec:
    """Store validated execution spec fields used by toolkit reports and adapters."""
    architecture: str
    code_base: int
    stack_base: int
    stack_size: int
...
def load_execution_spec(path: Path) -> ExecutionSpec:
    """Load execution spec for the current toolkit workflow."""
...
def _register_map(architecture: str, x86_const: Any) -> dict[str, int]:
    """Support register map processing for internal toolkit callers."""
    names32 = [
...
def _map_region(uc: Any, mapped: list[tuple[int, int]], address: int, size: int, permissions: int) -> None:
    """Support map region processing for internal toolkit callers."""
...
def execute_code(code: bytes, spec: ExecutionSpec, *, code_base: int | None = None) -> dict[str, Any]:
    """Execute the execute code operation for the current toolkit workflow."""
    ...
    def code_hook(uc_obj: Any, address: int, size: int, _user_data: Any) -> None:
        """Execute the code hook operation for the current toolkit workflow."""
...
def differential_validate(
    ...
    *,
    ...
    """Execute the differential validate operation for the current toolkit workflow."""
...
def differential_validate_files(
    ...
    *,
    ...
    """Execute the differential validate files operation for the current toolkit workflow."""
