...
class ProjectCheck:
    """Store validated project check fields used by toolkit reports and adapters."""
    valid: bool
    failures: tuple[str, ...]
    warnings: tuple[str, ...]
...
class ProjectStateDatabase:
    """Coordinate project state database behavior for the current toolkit workflow."""
    def __init__(self, path: Path, *, toolkit_version: str = "0.7.10"):
    ...
    def integrity_check(self) -> list[str]:
        """Execute the integrity check operation for the current toolkit workflow."""
    ...
    def record_migration(
        ...
        *,
        ...
        """Record migration for the current toolkit workflow."""
    ...
    def snapshot(self, project_root: Path) -> dict[str, Any]:
        """Execute the snapshot operation for the current toolkit workflow."""
    ...
    def upsert_artifact_reference(self, reference: str, digest: str, kind: str, owner: str) -> None:
        """Execute the upsert artifact reference operation for the current toolkit workflow."""
        with self.transaction():
            self.connection.execute(
    ...
    def artifact_digests(self) -> set[str]:
        """Execute the artifact digests operation for the current toolkit workflow."""
...
def state_database_path(project_root: Path) -> Path:
    """Execute the state database path operation for the current toolkit workflow."""
...
def _migration_2_to_3(root: Path, project: dict[str, Any]) -> dict[str, Any]:
    """Support migration 2 to 3 processing for internal toolkit callers."""
    for directory in (
        "state",
        "artifacts",
...
def migrate_project(
    ...
    *,
    ...
    """Execute the migrate project operation for the current toolkit workflow."""
    root = project_root.resolve()
    ...
    if current == 1:
        ...
        plan.append("1->2")
    working = _migration_2_to_3(root, working) if not dry_run else {**working, "schema_version": 3, "toolkit_release": "0.7.10"}
    plan.append("2->3")
    if dry_run:
...
def check_project_state(project_root: Path) -> ProjectCheck:
    """Check project state for the current toolkit workflow."""
    root = project_root.resolve()
    failures: list[str] = []
    warnings: list[str] = []
...
def project_gc(project_root: Path, *, dry_run: bool = True) -> dict[str, Any]:
    """Execute the project gc operation for the current toolkit workflow."""
    root = project_root.resolve()
    project = load_json(root / "project.json")
