...
class Capsules:
    ...
    def create(self,name:str,output:str|Path,*,include:list[str],external_requirements:list[dict[str,Any]]|None=None,actor:str='analyst')->dict[str,Any]:
        ...
        manifest={'schema':'x86decomp.capsule.v1','toolkit_version':'0.7.10','project_root_name':self.store.project_root.name,'members':[{'path':n,'sha256':sha256_file(p),'size':p.stat().st_size} for n,p in sorted(members)],'external_requirements':external_requirements or [],'audit_tip':self.store.verify_audit_chain()['tip_hash']}
