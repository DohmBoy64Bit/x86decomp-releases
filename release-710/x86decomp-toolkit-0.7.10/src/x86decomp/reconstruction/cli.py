...
from x86decomp.cli_utils import emit_json, parse_json_arg

from x86decomp.contracts import ContractError

_json = parse_json_arg
...
def build_parser(*,prog:str='x86decomp-reconstruction')->argparse.ArgumentParser:
    ...
    parser=argparse.ArgumentParser(prog=prog,description='x86decomp-toolkit 0.7.10 project-scale reconstruction control plane')
...
def dispatch(args:argparse.Namespace)->Any:
    ...
    if args.group=='llm':
        ...
        if args.action=='prompt':
            record=llm_prompt_record(load_llm_job(Path(args.job))); writeparse_json_arg(Path(args.output),record); return record
        if args.action=='generate':
            result=generate_llm_candidate(Path(args.profile),Path(args.job),Path(args.output),overwrite=args.overwrite)
            if args.report: writeparse_json_arg(Path(args.report),result)
            return result
...
def main(argv:list[str]|None=None)->int:
    ...
    parser=build_parser()
    try: args=parser.parse_args(argv); emit_json(dispatch(args)); return 0
