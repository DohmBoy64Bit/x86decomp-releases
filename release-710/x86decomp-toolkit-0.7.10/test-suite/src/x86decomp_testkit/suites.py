"""Provide the installed test-suite implementation for the `x86decomp_testkit.suites` module."""
...
def _missing(adapter_results: list[ProbeResult], required: tuple[str, ...]) -> list[str]:
    """Support missing processing for internal toolkit callers."""
...
def _result(test_id: str, suite: str, passed: bool, summary: str, *, details: dict[str, Any] | None = None) -> TestResult:
    """Support result processing for internal toolkit callers."""
    now = utc_now()
    return TestResult(
        test_id=test_id,
        suite=suite,
...
def run_inventory_tests(
    ...
    """Run inventory tests for the current toolkit workflow."""
...
def run_cli_surface_tests(
    ...
    """Run cli surface tests for the current toolkit workflow."""
...
def _verify_manifest(toolkit_root: Path) -> tuple[bool, dict[str, Any]]:
    """Support verify manifest processing for internal toolkit callers."""
    import hashlib
...
def _validate_schemas(toolkit_root: Path) -> tuple[bool, dict[str, Any]]:
    """Support validate schemas processing for internal toolkit callers."""
    try:
...
def _validate_java(toolkit_root: Path) -> tuple[bool, dict[str, Any]]:
    """Support validate java processing for internal toolkit callers."""
    try:
        import javalang
    except ImportError as exc:
...
def _validate_skill(toolkit_root: Path) -> tuple[bool, dict[str, Any]]:
    """Support validate skill processing for internal toolkit callers."""
    try:
        import yaml
    except ImportError as exc:
...
def run_structural_tests(
    ...
    """Run structural tests for the current toolkit workflow."""
    results: list[TestResult] = []
...
def run_harness_self_tests(
    ...
    """Run harness self tests for the current toolkit workflow."""
...
def run_pytest_and_coverage(
    ...
    """Run pytest and coverage for the current toolkit workflow."""
    results: list[TestResult] = []
...
def run_packaging_tests(
    ...
    """Run packaging tests for the current toolkit workflow."""
