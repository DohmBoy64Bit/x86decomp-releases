"""Provide the installed test-suite implementation for the `x86decomp_testkit.adapters.detection` module."""
...
def _module_version(module_name: str) -> str | None:
    """Support module version processing for internal toolkit callers."""
...
def _run_version(executable: Path, args: tuple[str, ...], timeout: int = 15) -> tuple[str | None, str | None]:
    """Support run version processing for internal toolkit callers."""
    try:
...
def _windows_executable_suffixes() -> set[str]:
    """Support windows executable suffixes processing for internal toolkit callers."""
    return {".exe", ".bat", ".cmd", ".com"}


def _prefer_windows_executable(candidates: list[Path]) -> Path | None:
    """Support prefer windows executable processing for internal toolkit callers."""
...
def _candidate_from_root(root: Path, spec: AdapterSpec) -> Path | None:
    """Support candidate from root processing for internal toolkit callers."""
    if root.is_file():
        return root
...
def _known_windows_msvc_roots() -> list[Path]:
    """Support known windows msvc roots processing for internal toolkit callers."""
    roots: list[Path] = []
...
def _find_recursive(root: Path, names: Iterable[str], max_depth: int = 6) -> Path | None:
    """Support find recursive processing for internal toolkit callers."""
...
def _with_capabilities(spec: AdapterSpec, installed: bool, path: str | None = None, version: str | None = None, source: str | None = None, diagnostics: list[str] | None = None) -> ProbeResult:
    """Support with capabilities processing for internal toolkit callers."""
    return ProbeResult(
        spec.adapter_id,
        installed=installed,
        path=path,
...
def _detect_http_endpoint(spec: AdapterSpec, config: TestConfig) -> ProbeResult:
    """Support detect http endpoint processing for internal toolkit callers."""
...
def detect_adapter(spec: AdapterSpec, config: TestConfig) -> ProbeResult:
    """Execute the detect adapter operation for the current toolkit workflow."""
    diagnostics: list[str] = []
...
def detect_all(catalog: dict[str, AdapterSpec], config: TestConfig) -> list[ProbeResult]:
    """Execute the detect all operation for the current toolkit workflow."""
