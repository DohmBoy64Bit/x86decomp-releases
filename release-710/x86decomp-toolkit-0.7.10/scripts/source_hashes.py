...
def _is_ignored(relative: Path) -> bool:
    """Support is ignored processing for internal toolkit callers."""
...
def _digest(path: Path) -> str:
    """Support digest processing for internal toolkit callers."""
...
def manifest_text(root: Path) -> str:
    """Execute the manifest text operation for the current toolkit workflow."""
    root = root.resolve()
    return "".join(
...
def write_manifest(root: Path) -> Path:
    """Write manifest for the current toolkit workflow."""
    root = root.resolve()
...
def _read_manifest(manifest: Path) -> tuple[dict[str, str], list[str]]:
    """Support read manifest processing for internal toolkit callers."""
...
def verify_manifest(root: Path) -> dict[str, Any]:
    """Verify manifest for the current toolkit workflow."""
    root = root.resolve()
    manifest = root / "MANIFEST.sha256"
...
def generate_all(root: Path) -> dict[str, str]:
    """Generate all for the current toolkit workflow."""
    root = root.resolve()
...
def verify_all(root: Path) -> dict[str, Any]:
    """Verify all for the current toolkit workflow."""
    root = root.resolve()
    result = {
