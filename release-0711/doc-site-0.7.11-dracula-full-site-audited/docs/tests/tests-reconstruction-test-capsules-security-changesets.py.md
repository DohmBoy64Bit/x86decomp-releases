...
description: Test source page for tests/reconstruction/test_capsules_security_changesets.py.
---
...
```python
"""Verify the current toolkit behavior covered by `tests/reconstruction/test_capsules_security_changesets.py`."""
...
def test_capsule_is_deterministic_verifiable_and_reproducible(tmp_path: Path) -> None:
    """Verify capsule is deterministic verifiable and reproducible behavior."""
    ...
    inspected=api.inspect(capsule['archive_path']); assert inspected['manifest']['toolkit_version']=='0.7.11'
...
def test_library_recognition_never_silently_replaces_code(tmp_path: Path) -> None:
    """Verify library recognition never silently replaces code behavior."""
...
def test_security_scan_reports_without_modifying_behavior(tmp_path: Path) -> None:
    """Verify security scan reports without modifying behavior behavior."""
...
def test_semantic_changesets_detect_conflicts_and_verify_clean_merges(tmp_path: Path) -> None:
    """Verify semantic changesets detect conflicts and verify clean merges behavior."""
