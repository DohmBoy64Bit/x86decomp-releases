...
description: Test source page for tests/test_linker_metadata_corpus.py.
---
...
```python
"""Verify the current toolkit behavior covered by `tests/test_linker_metadata_corpus.py`."""
...
def test_bigobj_and_comdat_resolution() -> None:
    """Verify bigobj and comdat resolution behavior."""
...
def test_map_parser_and_layout_reconstruction(tmp_path: Path) -> None:
    """Verify map parser and layout reconstruction behavior."""
...
def test_target_specific_image_normalization(tmp_path: Path) -> None:
    """Verify target specific image normalization behavior."""
...
def test_msvc_rtti_unwind_and_static_initializer_recovery(tmp_path: Path) -> None:
    """Verify msvc rtti unwind and static initializer recovery behavior."""
...
def test_compiler_ground_truth_corpus(tmp_path: Path) -> None:
    """Verify compiler ground truth corpus behavior."""
...
def test_symbolic_memory_alias_model() -> None:
    """Verify symbolic memory alias model behavior."""
    ...
    if importlib.util.find_spec("angr") is None or importlib.util.find_spec("claripy") is None:
        ...
        with pytest.raises(ExternalToolError):
            angr_memory_alias_compare(target, candidate, harness)
        return
...
def test_real_clang_weak_external_and_tls_coff(tmp_path: Path) -> None:
    """Verify real clang weak external and tls coff behavior."""
...
def test_builtin_corpus_manifest_is_expanded_and_sources_compile(tmp_path: Path) -> None:
    """Verify builtin corpus manifest is expanded and sources compile behavior."""
...
def test_ground_truth_cross_report_comparison(tmp_path: Path) -> None:
    """Verify ground truth cross report comparison behavior."""
...
def test_image_profile_can_explicitly_normalize_debug_records(tmp_path: Path) -> None:
    """Verify image profile can explicitly normalize debug records behavior."""
