...
description: Test source page for test-suite/src/x86decomp_testkit/self_tests/test_adapter_detection_resolution.py.
---
...
def test_installed_adapter_never_prompts(tmp_path: Path) -> None:
    """Verify installed adapter never prompts behavior."""
...
def test_missing_adapter_prompts_custom_path_then_accepts(tmp_path: Path) -> None:
    """Verify missing adapter prompts custom path then accepts behavior."""
...
def test_missing_noninteractive_is_explicit_unresolved(tmp_path: Path) -> None:
    """Verify missing noninteractive is explicit unresolved behavior."""
...
def test_environment_and_configured_root_detection(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Verify environment and configured root detection behavior."""
    root = tmp_path / "root"
    tool = root / "bin" / "tool"
...
def test_path_detection_preserves_symlink_argv0(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Verify that adapter detection keeps a symlinked argv0 path when supported."""
    if os.name == "nt":
        try:
...
def test_missing_python_adapter_accepts_custom_interpreter(tmp_path: Path) -> None:
    """Verify missing python adapter accepts custom interpreter behavior."""
