...
description: Test source page for tests/test_evidence.py.
---
...
```python
"""Verify the current toolkit behavior covered by `tests/test_evidence.py`."""
...
class EvidenceTests(unittest.TestCase):
    ...
    def test_same_group_does_not_verify(self) -> None:
        """Verify same group does not verify behavior."""
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            store = EvidenceStore(root)
            ids = []
