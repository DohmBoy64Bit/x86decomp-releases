...
description: Test source page for test-suite/tests/test_architecture_maps.py.
---
...
```python
"""Provide test-suite.tests.test_architecture_maps functionality for the x86decomp-toolkit 0.7.11 release.
...
def test_all_architecture_map_artifacts_exist_and_are_versioned() -> None:
    ...
    for relative in REQUIRED_MAPS:
        ...
        assert "0.7.11" in text, f"architecture version missing from {relative}"
