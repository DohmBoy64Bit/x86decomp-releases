...
description: Test source page for tests/test_local_llm.py.
---
...
```python
"""Verify the current toolkit behavior covered by `tests/test_local_llm.py`."""
...
def test_provider_profiles_and_loopback_policy(tmp_path: Path) -> None:
    """Verify provider profiles and loopback policy behavior."""
...
def test_prompt_is_deterministic_and_marks_injected_evidence_untrusted(tmp_path: Path) -> None:
    """Verify prompt is deterministic and marks injected evidence untrusted behavior."""
...
def test_openai_generation_and_structured_output_fallback(tmp_path: Path) -> None:
    """Verify openai generation and structured output fallback behavior."""
...
def test_ollama_probe_accepts_latest_tag_only_for_ollama(tmp_path: Path) -> None:
    """Verify ollama probe accepts latest tag only for ollama behavior."""
...
def test_same_origin_redirect_rejects_cross_origin(tmp_path: Path) -> None:
    """Verify same origin redirect rejects cross origin behavior."""
...
def test_exact_byte_match_loop_and_tamper_detection(tmp_path: Path) -> None:
    """Verify exact byte match loop and tamper detection behavior."""
...
def test_candidate_contract_rejects_inline_assembly_and_multiple_functions(tmp_path: Path) -> None:
    """Verify candidate contract rejects inline assembly and multiple functions behavior."""
