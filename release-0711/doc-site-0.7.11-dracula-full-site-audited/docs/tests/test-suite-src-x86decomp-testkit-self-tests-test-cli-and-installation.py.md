...
description: Test source page for test-suite/src/x86decomp_testkit/self_tests/test_cli_and_installation.py.
---
...
def test_cli_init_config_catalog_and_missing_config(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Verify cli init config catalog and missing config behavior."""
...
def test_install_python_command_and_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Verify install python command and failure behavior."""
