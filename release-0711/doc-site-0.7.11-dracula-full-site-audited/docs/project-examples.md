...
> **Mode model.** The 0.7.11 project schema permits only `matching` and `functional` in `selected_modes`. A target decision of `preferred_mode: both` enables both; “hybrid” describes their build/workflow composition, not a third enum.
