...
A concise reference of rules enforced by the toolkit.
...
* Loopback-only endpoints unless remote disclosure is explicitly enabled.
* Environment-variable secret lookup rather than persisted secret values.
* Same-origin redirects and bounded HTTP responses.
* Exactly one validated C function proposal.
* Deterministic compilation and explicit relocation resolution.
* Raw byte identity before any `byte_matched` claim.
* No automatic workflow-state promotion.
