...
def _store(args: argparse.Namespace) -> ReconstructionStore:
    """Open the project store requested by parsed command arguments."""
    return ReconstructionStore(args.project)
...
def build_parser(*,prog:str='x86decomp-reconstruction')->argparse.ArgumentParser:
    ...
    parser=argparse.ArgumentParser(prog=prog,description='x86decomp-toolkit 0.7.11 project-scale reconstruction control plane')
    ...
    q=cas.add_parser('search', description='emit an ordered candidate-search plan only; no source edits or promotions are performed'); q.add_argument('--phase',action='append'); q.add_argument('--output')
    ...
    q=tys.add_parser('propagate', description='emit a type-propagation plan only; no source edits are performed'); q.add_argument('--output')
    ...
    q=trs.add_parser('next', description='emit a triage plan only; no workflow state is modified'); q.add_argument('--goal',default='matching',choices=['matching','playable']); q.add_argument('--limit',type=int,default=25); q.add_argument('--output')
...
def dispatch(args:argparse.Namespace)->Any:
    ...
    if args.group=='project':
        ...
        if args.action=='synthesize-layout': return api.synthesize(parse_json_arg(args.inventory_json,[]),actor=actor)
    ...
    if args.group=='module':
        api=ProjectLayout(store)
        if args.action=='create': return api.create_module(args.name,kind=args.kind,source_path=args.source_path,confidence=args.confidence,evidence=parse_json_arg(args.evidence_json,[]),actor=actor)
        if args.action=='add-member': return api.add_member(args.module_id,args.member_kind,args.member_id,ordinal=args.ordinal,evidence=parse_json_arg(args.evidence_json,[]),actor=actor)
        if args.action=='show': return api.show_module(args.module_id)
        if args.action=='list': return api.list_modules()
        if args.action=='create-unit': return api.create_translation_unit(args.source_path,module_id=args.module_id,language=args.language,confidence=args.confidence,evidence=parse_json_arg(args.evidence_json,[]),actor=actor)
    ...
    if args.group=='headers':
        ...
        if args.action=='declare': return api.declare(args.header_id,args.symbol_id,args.declaration,kind=args.kind,confidence=args.confidence,evidence=parse_json_arg(args.evidence_json,[]),actor=actor)
    ...
    if args.group=='build':
        ...
        if args.action=='create': return api.create(args.name,mode=args.mode,generator=args.generator,output_root=args.output_root,metadata=parse_json_arg(args.metadata_json,{}),actor=actor)
        if args.action=='add-target': return api.add_target(args.build_id,args.name,kind=args.kind,output_name=args.output_name,sources=parse_json_arg(args.sources_json,[]),dependencies=parse_json_arg(args.dependencies_json,[]),actor=actor)
        if args.action=='add-variant': return api.add_variant(args.target_id,args.name,compiler=args.compiler,linker=args.linker,compile_flags=parse_json_arg(args.compile_flags_json,[]),link_flags=parse_json_arg(args.link_flags_json,[]),environment=parse_json_arg(args.environment_json,{}),actor=actor)
    ...
    if args.group=='provenance':
        ...
        if args.action=='record': return api.record(args.source_path,args.line_start,args.line_end,args.binary_id,args.address_start,args.address_end,evidence=parse_json_arg(args.evidence_json,[]),confidence=args.confidence,actor=actor)
    ...
    if args.group=='abi':
        ...
        if args.action=='recover': return api.recover(args.subject_kind,args.subject_id,args.architecture,parse_json_arg(args.contract_json,{}),evidence=parse_json_arg(args.evidence_json,[]),actor=actor)
    ...
    if args.group=='tests':
        ...
        if args.action=='add': return api.add(args.name,args.scope_kind,args.scope_id,args.test_kind,args.relative_path,Path(args.content_file).read_text(encoding='utf-8'),applicability=parse_json_arg(args.applicability_json,{}),evidence=parse_json_arg(args.evidence_json,[]),actor=actor)
        if args.action=='list': return api.list()
        return api.explain(args.test_id)
    if args.group=='capsule':
        api=Capsules(store)
        if args.action=='create': return api.create(args.name,args.output,include=args.include,external_requirements=parse_json_arg(args.external_json,[]),actor=actor)
    ...
    if args.group=='library':
        ...
        if args.action=='identify': return api.identify(args.subject_id,args.library_name,version_range=args.version_range,confidence=args.confidence,evidence=parse_json_arg(args.evidence_json,[]),actor=actor)
    ...
    if args.group=='security':
        ...
        if args.action=='scan': return api.scan(parse_json_arg(args.observations_json,[]),actor=actor)
        if args.action=='finding': return api.finding(args.rule_id,args.severity,args.subject_id,args.summary,evidence=parse_json_arg(args.evidence_json,[]),actor=actor)
        if args.action=='policy': return api.policy(args.name,parse_json_arg(args.policy_json,{}),actor=actor)
        return api.report()
    if args.group=='llm':
    ...
    if args.group=='changeset':
        ...
        if args.action=='add-operation': return api.add_operation(args.changeset_id,parse_json_arg(args.operation_json,{}),actor=actor)
...
def main(argv: list[str] | None = None) -> int:
    """Run the reconstruction command-line entry point and return its exit status.
