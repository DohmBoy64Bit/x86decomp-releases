...
- **Inventory ID:** `F0393`
- **Relative path:** `src/x86decomp/pdb.py`
- **SHA-256:** `7f63eb4a225f26e469101a80227fc85d0cfafeda23c9ee6db9dac401de8a765d`
- **File size:** 25590 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Bounded MSF 7.0 / PDB inventory and PE identity correlation.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `src/x86decomp/target_pack.py`, `src/x86decomp/test_bundle.py`, `tests/test_pdb.py`
- **Key imports/dependencies:** `.binary_reader`, `.errors`, `.pe`, `.util`, `__future__`, `dataclasses`, `math`, `pathlib`, `struct`, `typing`, `uuid`

## B. Functional Summary

Bounded MSF 7.0 / PDB inventory and PE identity correlation. Major symbols: PDBStream, PDBFile, parse_pdb_bytes, parse_pdb, PDBStream.to_dict, PDBFile.to_dict, _MSF.stream. No side-effect-relevant call matched the audit scanner’s rule set.
...
Module docstring present: yes. Documented definitions: 24/24. 6 generic low-information docstring pattern hit(s) are enumerated in evidence/generic-docstrings.json and tracked by DOC-001.

Documentation was judged proportionally: public/non-obvious behavior requires specific contracts, while trivial fixtures and data files do not require artificial prose.

## E. Code Quality and Maintainability

Highest structural complexity: _MSF._parse_directory (branches=9, AST nodes=422), _MSF.__init__ (branches=9, AST nodes=224), _parse_source_info (branches=7, AST nodes=411), _parse_modules (branches=7, AST nodes=320), _parse_dbi (branches=6, AST nodes=626).
...
Static search found direct symbol references in tests: PDBFile(1), PDBStream(1), parse_pdb(3), parse_pdb_bytes(3), stream(1), to_dict(104). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: PDBFile(1), PDBStream(1), parse_pdb(3), parse_pdb_bytes(3), stream(1), to_dict(104). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
