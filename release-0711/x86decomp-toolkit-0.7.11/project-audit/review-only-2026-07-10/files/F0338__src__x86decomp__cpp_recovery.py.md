# Per-file audit: `src/x86decomp/cpp_recovery.py`

## A. File Identity

- **Inventory ID:** `F0338`
- **Relative path:** `src/x86decomp/cpp_recovery.py`
- **SHA-256:** `b0802ab4d397c31942d31760a5e09f5dd1da776b17dd015b3b0a76cc5b4bb7e6`
- **File size:** 8079 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Bounded C++ relationship recovery from MSVC metadata and code patterns.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `src/x86decomp/reconstruction/acceleration.py`, `test-suite/src/x86decomp_testkit/toolkit_tests/test_public_api_contract.py`, `tests/test_production.py`
- **Key imports/dependencies:** `.disassembly`, `.errors`, `.msvc_metadata`, `.util`, `__future__`, `pathlib`, `typing`

## B. Functional Summary

Bounded C++ relationship recovery from MSVC metadata and code patterns. Major symbols: recover_cpp_model. No side-effect-relevant call matched the audit scanner’s rule set.
...
Highest structural complexity: recover_cpp_model (branches=26, AST nodes=883), _adjustor_thunk_candidate (branches=6, AST nodes=185), _function_prefix (branches=3, AST nodes=63).
...
Static search found direct symbol references in tests: recover_cpp_model(3). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: recover_cpp_model(3). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
