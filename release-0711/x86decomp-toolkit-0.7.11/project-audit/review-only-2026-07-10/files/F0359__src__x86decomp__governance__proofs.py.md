# Per-file audit: `src/x86decomp/governance/proofs.py`

## A. File Identity

- **Inventory ID:** `F0359`
- **Relative path:** `src/x86decomp/governance/proofs.py`
- **SHA-256:** `fd94011a7c6e13042424cf7465b90da65aedd221130e4059062b38cf0b14b5ed`
- **File size:** 11198 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.governance.proofs` module.
...
- **Key imports/dependencies:** `.store`, `__future__`, `hashlib`, `hmac`, `json`, `os`, `pathlib`, `stat`, `tempfile`, `typing`, `x86decomp.contracts`, `zipfile`

## B. Functional Summary

Provide the current runtime implementation for the `x86decomp.governance.proofs` module. Major symbols: ProofLedger, ProofBundle, ProofLedger.create_obligation, ProofLedger.add_result, ProofLedger.obligation, ProofLedger.result, ProofLedger.evaluate, ProofBundle.export, ProofBundle.verify, ProofBundle.inspect. Static analysis recorded 9 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: connection.execute×7, output.parent.mkdir×1, os.replace×1.
...
Highest structural complexity: ProofBundle.verify (branches=15, AST nodes=503), ProofBundle.export (branches=11, AST nodes=518), ProofLedger.add_result (branches=3, AST nodes=173), ProofLedger.create_obligation (branches=3, AST nodes=151), ProofLedger.obligation (branches=2, AST nodes=127). 14 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: ProofBundle(12), ProofLedger(8), add_result(2), create_obligation(2), evaluate(5), export(33), inspect(24), obligation(5), result(301), verify(45). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: ProofBundle(12), ProofLedger(8), add_result(2), create_obligation(2), evaluate(5), export(33), inspect(24), obligation(5), result(301), verify(45). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
