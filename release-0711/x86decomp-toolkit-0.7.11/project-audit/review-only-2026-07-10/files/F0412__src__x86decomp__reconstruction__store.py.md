# Per-file audit: `src/x86decomp/reconstruction/store.py`

## A. File Identity

- **Inventory ID:** `F0412`
- **Relative path:** `src/x86decomp/reconstruction/store.py`
- **SHA-256:** `847c7131a08623fa7087c4c2d90289f03851e678e94196514cd2700716983282`
- **File size:** 10642 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.reconstruction.store` module.
...
- **Key imports/dependencies:** `__future__`, `typing`, `x86decomp.contracts`, `x86decomp.governance.store`, `x86decomp.store_utils`

## B. Functional Summary

Provide the current runtime implementation for the `x86decomp.reconstruction.store` module. Major symbols: ReconstructionStore, ReconstructionStore.initialize, ReconstructionStore.check, ReconstructionStore.decode. Static analysis recorded 6 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Highest structural complexity: ReconstructionStore.check (branches=2, AST nodes=133), ReconstructionStore.initialize (branches=2, AST nodes=92), ReconstructionStore.decode (branches=0, AST nodes=32). 5 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: ReconstructionStore(16), check(67), decode(8), initialize(18). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: ReconstructionStore(16), check(67), decode(8), initialize(18). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
