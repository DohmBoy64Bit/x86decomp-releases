# Per-file audit: `src/x86decomp/native/store.py`

## A. File Identity

- **Inventory ID:** `F0388`
- **Relative path:** `src/x86decomp/native/store.py`
- **SHA-256:** `88028dd1a13e9dc97af9254c3174b5fc45e2460cf3120b54000ab2a94dad3b8d`
- **File size:** 6518 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.native.store` module.
...
- **Key imports/dependencies:** `__future__`, `typing`, `x86decomp.contracts`, `x86decomp.reconstruction.store`, `x86decomp.store_utils`

## B. Functional Summary

Provide the current runtime implementation for the `x86decomp.native.store` module. Major symbols: NativeStore, NativeStore.initialize, NativeStore.check, NativeStore.decode. Static analysis recorded 5 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Highest structural complexity: NativeStore.initialize (branches=3, AST nodes=108), NativeStore.check (branches=2, AST nodes=133), NativeStore.decode (branches=0, AST nodes=32).
...
Static search found direct symbol references in tests: NativeStore(21), check(67), decode(8), initialize(18). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
Normalized-AST duplicate group 3: src/x86decomp/assembly/store.py:AssemblyStore.decode, src/x86decomp/native/store.py:NativeStore.decode, src/x86decomp/reconstruction/store.py:ReconstructionStore.decode. Consolidation safety is assessed in CROSS_FILE_ANALYSIS.md.
...
- **Test confidence:** Static search found direct symbol references in tests: NativeStore(21), check(67), decode(8), initialize(18). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
