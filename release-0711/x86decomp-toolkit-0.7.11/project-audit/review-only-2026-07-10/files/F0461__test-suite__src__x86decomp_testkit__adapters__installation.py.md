# Per-file audit: `test-suite/src/x86decomp_testkit/adapters/installation.py`

## A. File Identity

- **Inventory ID:** `F0461`
- **Relative path:** `test-suite/src/x86decomp_testkit/adapters/installation.py`
- **SHA-256:** `e37f333a1d3f6c31ff46043cf4afd37fcb7f498d011526ebc6366fbc3d469836`
- **File size:** 18255 bytes
- **Language/format:** Python source
- **Classification:** First-party verification-harness source
- **Apparent responsibility:** Provide the installed test-suite implementation for the `x86decomp_testkit.adapters.installation` module.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `test-suite/src/x86decomp_testkit/adapters/__init__.py`, `test-suite/src/x86decomp_testkit/self_tests/test_adapter_detection_resolution.py`, `test-suite/tests/test_adapter_detection_resolution.py`
- **Key imports/dependencies:** `..config`, `..logging_utils`, `..models`, `.detection`, `.download`, `__future__`, `os`, `pathlib`, `shutil`, `subprocess`, `typing`

## B. Functional Summary

Provide the installed test-suite implementation for the `x86decomp_testkit.adapters.installation` module. Major symbols: install_python_adapter, install_github_release, install_package_manager_adapter, install_adapter, resolve_missing_adapters. Static analysis recorded 6 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: shutil.rmtree×2, subprocess.run×1, destination.mkdir×1, temporary.mkdir×1, temporary.rename×1. 2 broad exception handler(s) were inspected in context; no blanket finding was created without a demonstrated failure path.
...
Highest structural complexity: resolve_missing_adapters (branches=35, AST nodes=741), install_github_release (branches=14, AST nodes=445), _validate_custom_path (branches=9, AST nodes=317), install_package_manager_adapter (branches=9, AST nodes=221), _automatic_install_available (branches=4, AST nodes=63). 2 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
resolve_missing_adapters has 35 branch/compound nodes; treat this as a profiling and maintainability target, not a confirmed performance defect.
