# Per-file audit: `test-suite/src/x86decomp_testkit/suites.py`

## A. File Identity

- **Inventory ID:** `F0482`
- **Relative path:** `test-suite/src/x86decomp_testkit/suites.py`
- **SHA-256:** `cc67eed42b7882a1ca0155a880fab4576d6a7254d040e3ecec3993359c6f68b5`
- **File size:** 26593 bytes
- **Language/format:** Python source
- **Classification:** First-party verification-harness source
- **Apparent responsibility:** Provide the installed test-suite implementation for the `x86decomp_testkit.suites` module.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `test-suite/src/x86decomp_testkit/orchestrator.py`
- **Key imports/dependencies:** `.adapters`, `.config`, `.coverage_audit`, `.inventory`, `.junit`, `.logging_utils`, `.models`, `.process`, `.timeutil`, `__future__`, `hashlib`, `javalang`, `json`, `jsonschema.validators`, `os`, `pathlib`, `shutil`, `typing`, `yaml`

## B. Functional Summary

Provide the installed test-suite implementation for the `x86decomp_testkit.suites` module. Major symbols: run_inventory_tests, run_cli_surface_tests, run_structural_tests, run_harness_self_tests, run_pytest_and_coverage, run_packaging_tests. Static analysis recorded 3 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: shutil.rmtree×2, dist.mkdir×1. 2 broad exception handler(s) were inspected in context; no blanket finding was created without a demonstrated failure path.
...
Highest structural complexity: run_packaging_tests (branches=8, AST nodes=421), _verify_manifest (branches=7, AST nodes=208), run_pytest_and_coverage (branches=6, AST nodes=693), _validate_skill (branches=6, AST nodes=188), _missing (branches=6, AST nodes=111). 13 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
