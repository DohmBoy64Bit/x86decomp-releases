# Per-file audit: `src/x86decomp/governance/family.py`

## A. File Identity

- **Inventory ID:** `F0355`
- **Relative path:** `src/x86decomp/governance/family.py`
- **SHA-256:** `b3f1c7659e7b18c52a5d84fa1c1d5aebeb33df451e1a3b41630c133670c7e62d`
- **File size:** 7285 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.governance.family` module.
...
Provide the current runtime implementation for the `x86decomp.governance.family` module. Major symbols: BinaryFamilyStore, BinaryFamilyStore.create, BinaryFamilyStore.add, BinaryFamilyStore.correlate, BinaryFamilyStore.get, BinaryFamilyStore.member, BinaryFamilyStore.correlation, BinaryFamilyStore.report. Static analysis recorded 11 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: connection.execute×9, destination.parent.mkdir×1, destination.write_bytes×1.

Boundary conditions, malformed input, error propagation, resource cleanup, path handling, encoding, integer/size limits, and partial-operation behavior were assessed where applicable. Findings are listed in Section L; where no finding is listed, the audit did not establish a defect rather than proving none exists.

## D. Documentation and Docstring Review

Module docstring present: yes. Documented definitions: 10/10. 7 generic low-information docstring pattern hit(s) are enumerated in evidence/generic-docstrings.json and tracked by DOC-001.

Documentation was judged proportionally: public/non-obvious behavior requires specific contracts, while trivial fixtures and data files do not require artificial prose.

## E. Code Quality and Maintainability

Highest structural complexity: BinaryFamilyStore.add (branches=5, AST nodes=260), BinaryFamilyStore.report (branches=2, AST nodes=119), BinaryFamilyStore._block_fingerprints (branches=2, AST nodes=71), BinaryFamilyStore.correlate (branches=1, AST nodes=346), BinaryFamilyStore.create (branches=1, AST nodes=103). 14 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: BinaryFamilyStore(12), add(30), correlate(3), correlation(2), create(65), get(100), member(38), report(211). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: BinaryFamilyStore(12), add(30), correlate(3), correlation(2), create(65), get(100), member(38), report(211). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
