# Per-file audit: `test-suite/src/x86decomp_testkit/inventory.py`

## A. File Identity

- **Inventory ID:** `F0467`
- **Relative path:** `test-suite/src/x86decomp_testkit/inventory.py`
- **SHA-256:** `88623c4fb82e7ff06e9ea219f9c20a2bf1944bd983a784cf2144a68ca1ee5e4d`
- **File size:** 11628 bytes
- **Language/format:** Python source
- **Classification:** First-party verification-harness source
- **Apparent responsibility:** Provide the installed test-suite implementation for the `x86decomp_testkit.inventory` module.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `scripts/sync-release-contracts.py`, `test-suite/src/x86decomp_testkit/cli.py`, `test-suite/src/x86decomp_testkit/coverage_audit.py`, `test-suite/src/x86decomp_testkit/self_tests/test_inventory_reports_process.py`, `test-suite/src/x86decomp_testkit/suites.py`, `test-suite/tests/test_inventory_reports_process.py`, `tests/test_release_contract.py`
- **Key imports/dependencies:** `__future__`, `ast`, `dataclasses`, `json`, `os`, `pathlib`, `re`, `subprocess`, `sys`, `typing`

## B. Functional Summary

Provide the installed test-suite implementation for the `x86decomp_testkit.inventory` module. Major symbols: PublicSymbol, discover_public_symbols, discover_all_function_symbols, discover_modules, discover_schemas, discover_ghidra_scripts, discover_cli_commands, build_inventory, load_feature_catalog, audit_catalog, PublicSymbol.symbol_id, PublicSymbol.to_dict. Static analysis recorded 1 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Module docstring present: yes. Documented definitions: 16/16. 1 generic low-information docstring pattern hit(s) are enumerated in evidence/generic-docstrings.json and tracked by DOC-001.

Documentation was judged proportionally: public/non-obvious behavior requires specific contracts, while trivial fixtures and data files do not require artificial prose.

## E. Code Quality and Maintainability

Highest structural complexity: _discover_symbols (branches=13, AST nodes=257), discover_cli_commands (branches=3, AST nodes=144), _direct_body_lines (branches=3, AST nodes=93), build_inventory (branches=2, AST nodes=94), _python_files (branches=2, AST nodes=64).
