# Per-file audit: `src/x86decomp/native/matching.py`

## A. File Identity

- **Inventory ID:** `F0383`
- **Relative path:** `src/x86decomp/native/matching.py`
- **SHA-256:** `fada98a90d0cdebc0e68c89511b46149d98807943863bd6e334af7be85ebb9e0`
- **File size:** 8814 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.native.matching` module.
...
- **Key imports/dependencies:** `.store`, `__future__`, `pathlib`, `typing`, `x86decomp.coff`, `x86decomp.contracts`, `x86decomp.pe`, `x86decomp.util`

## B. Functional Summary

Provide the current runtime implementation for the `x86decomp.native.matching` module. Major symbols: rva_to_file_offset, extract_candidate, compare_function_bytes, FunctionMatching, FunctionMatching.batch, FunctionMatching.report, FunctionMatching.mismatches. Static analysis recorded 5 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: connection.execute×5.

Boundary conditions, malformed input, error propagation, resource cleanup, path handling, encoding, integer/size limits, and partial-operation behavior were assessed where applicable. Findings are listed in Section L; where no finding is listed, the audit did not establish a defect rather than proving none exists.

## D. Documentation and Docstring Review

Module docstring present: yes. Documented definitions: 8/8. 7 generic low-information docstring pattern hit(s) are enumerated in evidence/generic-docstrings.json and tracked by DOC-001.

Documentation was judged proportionally: public/non-obvious behavior requires specific contracts, while trivial fixtures and data files do not require artificial prose.

## E. Code Quality and Maintainability

Highest structural complexity: compare_function_bytes (branches=16, AST nodes=357), FunctionMatching.batch (branches=11, AST nodes=766), rva_to_file_offset (branches=3, AST nodes=74), FunctionMatching.report (branches=2, AST nodes=104), extract_candidate (branches=2, AST nodes=68). 4 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: FunctionMatching(12), batch(19), compare_function_bytes(5), extract_candidate(1), mismatches(5), report(211), rva_to_file_offset(2). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: FunctionMatching(12), batch(19), compare_function_bytes(5), extract_candidate(1), mismatches(5), report(211), rva_to_file_offset(2). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
