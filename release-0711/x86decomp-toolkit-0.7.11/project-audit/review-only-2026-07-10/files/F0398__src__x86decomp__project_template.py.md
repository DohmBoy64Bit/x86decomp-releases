# Per-file audit: `src/x86decomp/project_template.py`

## A. File Identity

- **Inventory ID:** `F0398`
- **Relative path:** `src/x86decomp/project_template.py`
- **SHA-256:** `973cad988eb1648ed6fe3de38eb57b392bc88f6ce878cac223d2e68d57597de4`
- **File size:** 8743 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Grounded project-template generation from a verified target pack.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `src/x86decomp/target_pack.py`, `tests/test_production.py`
- **Key imports/dependencies:** `.errors`, `.target_pack`, `.util`, `__future__`, `pathlib`, `typing`

## B. Functional Summary

Grounded project-template generation from a verified target pack. Major symbols: derive_template_contract, materialize_project_template. Static analysis recorded 3 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: path.parent.mkdir×1, path.write_text×1, path.chmod×1.
...
Highest structural complexity: derive_template_contract (branches=9, AST nodes=462), materialize_project_template (branches=4, AST nodes=266), _artifact_roles (branches=1, AST nodes=50), _write_project_helper (branches=0, AST nodes=41). 2 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: derive_template_contract(3), materialize_project_template(3). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: derive_template_contract(3), materialize_project_template(3). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
