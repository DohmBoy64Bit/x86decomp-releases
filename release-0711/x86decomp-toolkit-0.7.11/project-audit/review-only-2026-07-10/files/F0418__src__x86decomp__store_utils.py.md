# Per-file audit: `src/x86decomp/store_utils.py`

## A. File Identity

- **Inventory ID:** `F0418`
- **Relative path:** `src/x86decomp/store_utils.py`
- **SHA-256:** `79ca047c185646cc9cd5fb8493b8c4024cab512fbafe2a4687971a7c650113fb`
- **File size:** 777 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Shared persistence helpers for SQLite-backed toolkit stores.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/assembly/store.py`, `src/x86decomp/native/store.py`, `src/x86decomp/reconstruction/store.py`, `tests/test_audit_fixes.py`
- **Key imports/dependencies:** `__future__`, `json`, `typing`

## B. Functional Summary

Shared persistence helpers for SQLite-backed toolkit stores. Major symbols: decode_json_fields. No side-effect-relevant call matched the audit scanner’s rule set.
...
Highest structural complexity: decode_json_fields (branches=2, AST nodes=89).
...
Static search found direct symbol references in tests: decode_json_fields(3). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: decode_json_fields(3). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
