...
- **Inventory ID:** `F0279`
- **Relative path:** `scripts/run-pytest-partitions.py`
- **SHA-256:** `62226bfbb34caa2de44f5b01bc0884de0e3ef40e9be128fd30465b4786d48168`
...
- **Key imports/dependencies:** `__future__`, `argparse`, `collections`, `json`, `os`, `pathlib`, `subprocess`, `sys`, `tempfile`, `typing`, `xml.etree.ElementTree`

## B. Functional Summary

Repository python source asset. Major symbols: run, main. Static analysis recorded 4 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: subprocess.run×2, report_path.parent.mkdir×1, report_path.write_text×1.
...
Highest structural complexity: run (branches=8, AST nodes=513), _partitions (branches=4, AST nodes=155), _collect (branches=4, AST nodes=139), _junit_counts (branches=2, AST nodes=89), main (branches=1, AST nodes=189). 1 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
