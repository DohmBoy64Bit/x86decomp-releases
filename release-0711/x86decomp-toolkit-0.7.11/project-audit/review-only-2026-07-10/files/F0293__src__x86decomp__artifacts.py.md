# Per-file audit: `src/x86decomp/artifacts.py`

## A. File Identity

- **Inventory ID:** `F0293`
- **Relative path:** `src/x86decomp/artifacts.py`
- **SHA-256:** `edacfe3d11869e434bdd7cb0ab806a4e7714c1181604eae619d2d0cc9f9f36e8`
- **File size:** 4870 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Function artifact import and validation.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `src/x86decomp/decompme.py`, `src/x86decomp/reconstruction/acceleration.py`, `src/x86decomp/workflow.py`, `tests/test_artifacts.py`
- **Key imports/dependencies:** `.errors`, `.util`, `__future__`, `pathlib`, `re`, `shutil`, `typing`

## B. Functional Summary

Function artifact import and validation. Major symbols: function_id_from_rva, validate_function_manifest, import_function_artifact, verify_function_artifact. Static analysis recorded 1 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: shutil.rmtree×1.
...
Highest structural complexity: verify_function_artifact (branches=14, AST nodes=339), validate_function_manifest (branches=10, AST nodes=215), import_function_artifact (branches=7, AST nodes=234), function_id_from_rva (branches=1, AST nodes=32).
...
Static search found direct symbol references in tests: function_id_from_rva(1), import_function_artifact(4), validate_function_manifest(1), verify_function_artifact(4). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: function_id_from_rva(1), import_function_artifact(4), validate_function_manifest(1), verify_function_artifact(4). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
