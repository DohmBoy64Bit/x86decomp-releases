# Per-file audit: `tests/test_pe64_patch_hybrid.py`

## A. File Identity

- **Inventory ID:** `F0536`
- **Relative path:** `tests/test_pe64_patch_hybrid.py`
...
- **Key imports/dependencies:** `json`, `pathlib`, `pe_fixture`, `subprocess`, `x86decomp.hybrid`, `x86decomp.patching`, `x86decomp.pe`, `x86decomp.project`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_pe64_project, test_patch_image, test_hybrid_project_builds_assembly_objects. Static analysis recorded 2 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: candidate.write_bytes×1, subprocess.run×1.
...
Highest structural complexity: test_hybrid_project_builds_assembly_objects (branches=0, AST nodes=204), test_patch_image (branches=0, AST nodes=98), test_pe64_project (branches=0, AST nodes=75).
