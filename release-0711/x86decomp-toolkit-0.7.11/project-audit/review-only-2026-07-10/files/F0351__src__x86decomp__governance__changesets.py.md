# Per-file audit: `src/x86decomp/governance/changesets.py`

## A. File Identity

- **Inventory ID:** `F0351`
- **Relative path:** `src/x86decomp/governance/changesets.py`
- **SHA-256:** `0e77e6dd57ceba75e956978b856be378e95a5705bc04185a22f47a38fc7cc269`
- **File size:** 6149 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.governance.changesets` module.
...
- **Key imports/dependencies:** `.store`, `__future__`, `json`, `pathlib`, `stat`, `typing`, `x86decomp.contracts`, `zipfile`

## B. Functional Summary

Provide the current runtime implementation for the `x86decomp.governance.changesets` module. Major symbols: ChangeSet, ChangeSet.export, ChangeSet.inspect, ChangeSet.apply. Static analysis recorded 7 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: connection.execute×6, output.parent.mkdir×1.
...
Highest structural complexity: ChangeSet.inspect (branches=9, AST nodes=326), ChangeSet.export (branches=3, AST nodes=365), ChangeSet.apply (branches=3, AST nodes=245). 10 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: ChangeSet(8), apply(6), export(33), inspect(24). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: ChangeSet(8), apply(6), export(33), inspect(24). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
