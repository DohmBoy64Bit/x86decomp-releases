# Per-file audit: `tests/reconstruction/test_real_project_acceleration.py`

## A. File Identity

- **Inventory ID:** `F0513`
- **Relative path:** `tests/reconstruction/test_real_project_acceleration.py`
- **SHA-256:** `1bbe5f864faaca1df9c4503aff61db7be0511376152fca5fb0cc978431478559`
...
- **Key imports/dependencies:** `__future__`, `json`, `pathlib`, `struct`, `x86decomp.reconstruction.acceleration`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_universal_function_pattern_and_text_swap, test_project_health_hygiene_runtime_and_policy, test_toolchain_rules_and_cleanup. Static analysis recorded 11 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: path.write_bytes×1, raw.write_bytes×1, rep.write_text×1, fl.write_text×1, scan_path.write_text×1, repl.write_bytes×1, src.write_text×1, script.write_text×1, tool.mkdir×1, obs.write_text×1, decomp.write_text×1.
...
Highest structural complexity: test_universal_function_pattern_and_text_swap (branches=0, AST nodes=400), test_toolchain_rules_and_cleanup (branches=0, AST nodes=194), test_project_health_hygiene_runtime_and_policy (branches=0, AST nodes=187), _minimal_pe (branches=0, AST nodes=166). 2 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
