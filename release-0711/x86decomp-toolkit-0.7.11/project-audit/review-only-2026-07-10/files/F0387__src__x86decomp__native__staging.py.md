# Per-file audit: `src/x86decomp/native/staging.py`

## A. File Identity

- **Inventory ID:** `F0387`
- **Relative path:** `src/x86decomp/native/staging.py`
- **SHA-256:** `ae21143ee6a45cf524897ce32db40a694dd1afe7b06ef125c023de955bbd1f23`
- **File size:** 5111 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.native.staging` module.
...
- **Key imports/dependencies:** `.store`, `__future__`, `pathlib`, `re`, `subprocess`, `typing`, `x86decomp.contracts`, `x86decomp.util`

## B. Functional Summary

Provide the current runtime implementation for the `x86decomp.native.staging` module. Major symbols: StagingBridge, StagingBridge.scan, StagingBridge.generate_context, StagingBridge.resolve, StagingBridge.unresolved, StagingBridge.compile_check. Static analysis recorded 9 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: c.execute×6, output.parent.mkdir×1, output.write_text×1, subprocess.run×1.
...
Highest structural complexity: StagingBridge.scan (branches=6, AST nodes=207), StagingBridge.generate_context (branches=4, AST nodes=321), StagingBridge.resolve (branches=2, AST nodes=143), StagingBridge.compile_check (branches=2, AST nodes=129), StagingBridge.unresolved (branches=1, AST nodes=46). 8 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: StagingBridge(8), compile_check(2), generate_context(2), resolve(62), scan(15), unresolved(20). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: StagingBridge(8), compile_check(2), generate_context(2), resolve(62), scan(15), unresolved(20). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
