# Per-file audit: `src/x86decomp/synthetic_corpus.py`

## A. File Identity

- **Inventory ID:** `F0420`
- **Relative path:** `src/x86decomp/synthetic_corpus.py`
- **SHA-256:** `0f2dcfe167b0a0a0db7d9329e56d823aa57698c3aaa7ddb9d01546e3a98d5e5b`
- **File size:** 13599 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Deterministic ground-truth source corpus generation.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `tests/test_production.py`
- **Key imports/dependencies:** `.errors`, `.util`, `__future__`, `pathlib`, `random`, `re`, `typing`

## B. Functional Summary

Deterministic ground-truth source corpus generation. Major symbols: generate_synthetic_corpus, verify_synthetic_corpus. Static analysis recorded 2 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: source_root.mkdir×1, source.write_text×1.

Boundary conditions, malformed input, error propagation, resource cleanup, path handling, encoding, integer/size limits, and partial-operation behavior were assessed where applicable. Findings are listed in Section L; where no finding is listed, the audit did not establish a defect rather than proving none exists.

## D. Documentation and Docstring Review

Module docstring present: yes. Documented definitions: 15/15. 13 generic low-information docstring pattern hit(s) are enumerated in evidence/generic-docstrings.json and tracked by DOC-001.

Documentation was judged proportionally: public/non-obvious behavior requires specific contracts, while trivial fixtures and data files do not require artificial prose.

## E. Code Quality and Maintainability

Highest structural complexity: generate_synthetic_corpus (branches=15, AST nodes=569), verify_synthetic_corpus (branches=13, AST nodes=364), _symbol (branches=3, AST nodes=66), _c_branches (branches=2, AST nodes=115), _c_switch (branches=2, AST nodes=100).
...
Static search found direct symbol references in tests: generate_synthetic_corpus(4), verify_synthetic_corpus(4). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: generate_synthetic_corpus(4), verify_synthetic_corpus(4). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
