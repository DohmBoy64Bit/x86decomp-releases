# Per-file audit: `tests/assembly/test_helpers_and_relocations.py`

## A. File Identity

- **Inventory ID:** `F0495`
- **Relative path:** `tests/assembly/test_helpers_and_relocations.py`
...
- **Key imports/dependencies:** `__future__`, `pathlib`, `pytest`, `x86decomp.assembly`, `x86decomp.assembly.materialize`, `x86decomp.assembly.relocations`, `x86decomp.contracts`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_assembly_cli_json_integer_and_error_helpers, test_symbol_address_error_and_render_helpers, test_relocation_inspection_paths. Static analysis recorded 1 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: value.write_text×1.
...
Highest structural complexity: test_relocation_inspection_paths (branches=1, AST nodes=121), test_symbol_address_error_and_render_helpers (branches=1, AST nodes=115), test_assembly_cli_json_integer_and_error_helpers (branches=0, AST nodes=107).
