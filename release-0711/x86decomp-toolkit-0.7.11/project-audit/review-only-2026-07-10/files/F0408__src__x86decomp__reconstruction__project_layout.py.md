# Per-file audit: `src/x86decomp/reconstruction/project_layout.py`

## A. File Identity

- **Inventory ID:** `F0408`
- **Relative path:** `src/x86decomp/reconstruction/project_layout.py`
- **SHA-256:** `326422cac484c8476e5f800a19881f92b3f01ca85a7ace224908aa7cd96025ec`
- **File size:** 8797 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.reconstruction.project_layout` module.
...
Provide the current runtime implementation for the `x86decomp.reconstruction.project_layout` module. Major symbols: ProjectLayout, ProjectLayout.create_module, ProjectLayout.add_member, ProjectLayout.create_translation_unit, ProjectLayout.add_translation_unit_member, ProjectLayout.synthesize, ProjectLayout.show_module, ProjectLayout.show_translation_unit, ProjectLayout.list_modules, ProjectLayout.explain_boundaries, ProjectLayout.export. Static analysis recorded 18 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: c.execute×15, out.parent.mkdir×1, out.write_text×1, stem.replace×1.
...
Highest structural complexity: ProjectLayout.synthesize (branches=7, AST nodes=332), ProjectLayout.create_translation_unit (branches=6, AST nodes=232), ProjectLayout.create_module (branches=4, AST nodes=198), ProjectLayout.add_member (branches=3, AST nodes=175), ProjectLayout.add_translation_unit_member (branches=2, AST nodes=154). 21 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: ProjectLayout(13), add_member(1), add_translation_unit_member(2), create_module(1), create_translation_unit(2), explain_boundaries(2), export(33), list_modules(1), show_module(1), show_translation_unit(1). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
