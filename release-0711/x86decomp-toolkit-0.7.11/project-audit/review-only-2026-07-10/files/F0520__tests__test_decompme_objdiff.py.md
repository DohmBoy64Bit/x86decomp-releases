# Per-file audit: `tests/test_decompme_objdiff.py`

## A. File Identity

- **Inventory ID:** `F0520`
- **Relative path:** `tests/test_decompme_objdiff.py`
...
- **Key imports/dependencies:** `__future__`, `json`, `pathlib`, `sys`, `x86decomp.decompme`, `x86decomp.objdiff_adapter`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_create_local_decompme_packet, test_manifest_driven_objdiff_adapter. Static analysis recorded 5 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: artifact.mkdir×1, target.write_bytes×1, candidate.write_bytes×1, script.write_text×1, manifest.write_text×1.
...
Highest structural complexity: test_create_local_decompme_packet (branches=1, AST nodes=150), test_manifest_driven_objdiff_adapter (branches=0, AST nodes=135).
