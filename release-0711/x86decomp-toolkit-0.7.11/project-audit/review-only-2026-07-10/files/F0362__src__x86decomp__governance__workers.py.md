# Per-file audit: `src/x86decomp/governance/workers.py`

## A. File Identity

- **Inventory ID:** `F0362`
- **Relative path:** `src/x86decomp/governance/workers.py`
- **SHA-256:** `7b610e1c2a66841ee8922a3e8d0c899c36d143a7cc204f8b50b670e99ffec3d6`
- **File size:** 4724 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.governance.workers` module.
...
Provide the current runtime implementation for the `x86decomp.governance.workers` module. Major symbols: WorkerRegistry, WorkerRegistry.register, WorkerRegistry.get, WorkerRegistry.list, WorkerRegistry.select, WorkerRegistry.set_status, WorkerRegistry.doctor. Static analysis recorded 4 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: connection.execute×4.
...
Highest structural complexity: WorkerRegistry.select (branches=7, AST nodes=146), WorkerRegistry.register (branches=4, AST nodes=170), WorkerRegistry.list (branches=4, AST nodes=121), WorkerRegistry.doctor (branches=4, AST nodes=110), WorkerRegistry.set_status (branches=1, AST nodes=112). 7 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: WorkerRegistry(9), doctor(17), get(100), list(167), register(6), select(19), set_status(2). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: WorkerRegistry(9), doctor(17), get(100), list(167), register(6), select(19), set_status(2). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
