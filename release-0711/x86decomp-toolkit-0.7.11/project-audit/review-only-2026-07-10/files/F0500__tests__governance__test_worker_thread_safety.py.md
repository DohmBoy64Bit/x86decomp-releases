# Per-file audit: `tests/governance/test_worker_thread_safety.py`

## A. File Identity

- **Inventory ID:** `F0500`
- **Relative path:** `tests/governance/test_worker_thread_safety.py`
- **SHA-256:** `ad66aedbfb57d99a66c017113d32f2498c9d0f6de644b55e7d51a7372a0ee36c`
...
- **Key imports/dependencies:** `__future__`, `os`, `pathlib`, `pytest`, `subprocess`, `sys`, `types`, `x86decomp.worker`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_worker_uses_exec_wrapper_without_thread_unsafe_preexec, test_legacy_preexec_compatibility_callback_is_executable, test_worker_uses_exec_wrapper_without_thread_unsafe_preexec.recording_popen. No side-effect-relevant call matched the audit scanner’s rule set.
...
Highest structural complexity: test_legacy_preexec_compatibility_callback_is_executable (branches=1, AST nodes=221), test_worker_uses_exec_wrapper_without_thread_unsafe_preexec (branches=0, AST nodes=170), test_worker_uses_exec_wrapper_without_thread_unsafe_preexec.recording_popen (branches=0, AST nodes=41).
