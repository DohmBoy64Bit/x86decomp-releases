# Per-file audit: `tests/test_ghidra.py`

## A. File Identity

- **Inventory ID:** `F0527`
- **Relative path:** `tests/test_ghidra.py`
- **SHA-256:** `0f6ea9e4585a8d61ae0cb4a242e76e2b8005487190e428128230ae1cec0b0a9d`
...
- **Key imports/dependencies:** `__future__`, `pathlib`, `tempfile`, `unittest`, `x86decomp.ghidra`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: GhidraCommandTests, GhidraCommandTests.test_build_export_command_is_argument_array. Static analysis recorded 4 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: analyze.parent.mkdir×1, analyze.write_text×1, binary.write_bytes×1, scripts.mkdir×1.
...
Highest structural complexity: GhidraCommandTests.test_build_export_command_is_argument_array (branches=1, AST nodes=197).
...
Executed in the exact inventory: 1 collected test(s), all passing.
...
- **Test confidence:** Executed in the exact inventory: 1 collected test(s), all passing.
