# Per-file audit: `examples/local-llm/add-two-ints-job.json`

## A. File Identity

- **Inventory ID:** `F0078`
- **Relative path:** `examples/local-llm/add-two-ints-job.json`
- **SHA-256:** `aac7785ac0e8ccc9e408fe583c7a96ca8a4bb01abd51eb17c1553f127a8a86b5`
- **File size:** 741 bytes
- **Language/format:** JSON
- **Classification:** Example or sample
- **Apparent responsibility:** Structured repository data with top-level keys: schema_version, id, function_name, symbol, architecture, mnemonics, target_bytes_hex, base_rva.
...
Structured repository data with top-level keys: schema_version, id, function_name, symbol, architecture, mnemonics, target_bytes_hex, base_rva. JSON parsing was verified; top-level type is dict.
