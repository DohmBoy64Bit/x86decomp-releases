# Per-file audit: `tests/test_dynamorio.py`

## A. File Identity

- **Inventory ID:** `F0525`
- **Relative path:** `tests/test_dynamorio.py`
...
- **Key imports/dependencies:** `importlib.util`, `pathlib`, `pytest`, `subprocess`, `sys`, `x86decomp.angr_backend`, `x86decomp.dynamorio`, `x86decomp.errors`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_parse_drcov_text, test_angr_backend_has_explicit_optional_dependency_error. Static analysis recorded 2 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: log.write_text×1, subprocess.run×1.
...
Highest structural complexity: test_angr_backend_has_explicit_optional_dependency_error (branches=1, AST nodes=83), test_parse_drcov_text (branches=0, AST nodes=67).
