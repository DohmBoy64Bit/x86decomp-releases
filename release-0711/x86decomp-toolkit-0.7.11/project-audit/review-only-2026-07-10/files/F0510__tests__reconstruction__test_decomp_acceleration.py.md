# Per-file audit: `tests/reconstruction/test_decomp_acceleration.py`

## A. File Identity

- **Inventory ID:** `F0510`
- **Relative path:** `tests/reconstruction/test_decomp_acceleration.py`
- **SHA-256:** `2071a12ba3aae4739ec1cb64df4755b005411d5872103dd9fdf809745e706429`
...
- **Key imports/dependencies:** `__future__`, `json`, `pathlib`, `x86decomp.reconstruction.acceleration`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_llm_job_create_and_range, test_source_map_and_candidate_promotion, test_planning_helpers. Static analysis recorded 6 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: raw.write_bytes×1, src.mkdir×1, c.write_text×1, report.write_text×1, target.write_bytes×1, asset_root.mkdir×1.
...
Highest structural complexity: test_planning_helpers (branches=0, AST nodes=182), test_llm_job_create_and_range (branches=0, AST nodes=151), test_source_map_and_candidate_promotion (branches=0, AST nodes=151), _packet (branches=0, AST nodes=93). 1 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
