# Per-file audit: `src/x86decomp/governance/reviews.py`

## A. File Identity

- **Inventory ID:** `F0360`
- **Relative path:** `src/x86decomp/governance/reviews.py`
- **SHA-256:** `5415ea375c3491a438d4f9a4413433810bb20e64238e61f1ecb76250b2f845fd`
- **File size:** 6514 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.governance.reviews` module.
...
Provide the current runtime implementation for the `x86decomp.governance.reviews` module. Major symbols: ReviewQueue, ReviewQueue.create, ReviewQueue.assign, ReviewQueue.decide, ReviewQueue.lock, ReviewQueue.get, ReviewQueue.list. Static analysis recorded 11 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: connection.execute×11.
...
Highest structural complexity: ReviewQueue.decide (branches=6, AST nodes=234), ReviewQueue.list (branches=4, AST nodes=197), ReviewQueue.assign (branches=3, AST nodes=149), ReviewQueue.create (branches=2, AST nodes=142), ReviewQueue.get (branches=2, AST nodes=131). 7 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: ReviewQueue(10), assign(4), create(65), decide(4), get(100), list(167), lock(10). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: ReviewQueue(10), assign(4), create(65), decide(4), get(100), list(167), lock(10). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
