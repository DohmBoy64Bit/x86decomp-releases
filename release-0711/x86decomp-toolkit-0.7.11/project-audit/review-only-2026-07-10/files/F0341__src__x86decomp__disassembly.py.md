# Per-file audit: `src/x86decomp/disassembly.py`

## A. File Identity

- **Inventory ID:** `F0341`
- **Relative path:** `src/x86decomp/disassembly.py`
- **SHA-256:** `6ac452060750555a6808e2bb2a30577e519fc3d14ea4e75f92c3f2e4220fdf80`
- **File size:** 11481 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Independent Capstone-backed x86/x86-64 decoding and normalization.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/abi.py`, `src/x86decomp/assembly/annotation.py`, `src/x86decomp/cli.py`, `src/x86decomp/cpp_recovery.py`, `src/x86decomp/exe_diff.py`, `test-suite/src/x86decomp_testkit/toolkit_tests/test_public_api_contract.py`
- **Key imports/dependencies:** `.errors`, `.util`, `__future__`, `capstone`, `dataclasses`, `difflib`, `json`, `pathlib`, `typing`

## B. Functional Summary

Independent Capstone-backed x86/x86-64 decoding and normalization. Major symbols: InstructionRecord, decode_instructions, control_flow_edges, compare_instruction_streams, cross_check_ghidra_instructions, InstructionRecord.to_dict. No side-effect-relevant call matched the audit scanner’s rule set.
...
Highest structural complexity: decode_instructions (branches=22, AST nodes=725), cross_check_ghidra_instructions (branches=10, AST nodes=406), compare_instruction_streams (branches=7, AST nodes=347), control_flow_edges (branches=5, AST nodes=173), _is_known_address (branches=1, AST nodes=47).
...
Static search found direct symbol references in tests: InstructionRecord(1), compare_instruction_streams(3), control_flow_edges(3), cross_check_ghidra_instructions(3), decode_instructions(4), to_dict(104). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: InstructionRecord(1), compare_instruction_streams(3), control_flow_edges(3), cross_check_ghidra_instructions(3), decode_instructions(4), to_dict(104). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
