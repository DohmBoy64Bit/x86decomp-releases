# Per-file audit: `src/x86decomp/abi.py`

## A. File Identity

- **Inventory ID:** `F0290`
- **Relative path:** `src/x86decomp/abi.py`
- **SHA-256:** `7c5a0ebff5110f309a0a59f824d4c28a92ab31f61302c98462abd65828ab5c74`
- **File size:** 10731 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Explicit x86 ABI contracts and bounded static compatibility checks.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `src/x86decomp/harness_generator.py`, `test-suite/src/x86decomp_testkit/toolkit_tests/test_public_api_contract.py`, `tests/test_abi_disassembly.py`, `tests/test_production.py`
- **Key imports/dependencies:** `.disassembly`, `.errors`, `.util`, `__future__`, `dataclasses`, `enum`, `pathlib`, `typing`

## B. Functional Summary

Explicit x86 ABI contracts and bounded static compatibility checks. Major symbols: CallingConvention, FloatMode, ABIContract, load_abi_contract, analyze_abi, validate_abi, ABIContract.from_dict, ABIContract.from_dict.optional_nonnegative. No side-effect-relevant call matched the audit scanner’s rule set.
...
Highest structural complexity: analyze_abi (branches=21, AST nodes=440), ABIContract.from_dict (branches=16, AST nodes=367), validate_abi (branches=11, AST nodes=482), ABIContract.from_dict.optional_nonnegative (branches=3, AST nodes=66), load_abi_contract (branches=0, AST nodes=20).
...
Static search found direct symbol references in tests: ABIContract(7), CallingConvention(4), FloatMode(4), analyze_abi(1), from_dict(6), load_abi_contract(3), validate_abi(4). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: ABIContract(7), CallingConvention(4), FloatMode(4), analyze_abi(1), from_dict(6), load_abi_contract(3), validate_abi(4). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
