# Per-file audit: `src/x86decomp/reconstruction/generated_tests.py`

## A. File Identity

- **Inventory ID:** `F0405`
- **Relative path:** `src/x86decomp/reconstruction/generated_tests.py`
- **SHA-256:** `795acf06507747ba98e4e142daa8a8e424cdc860499aacdfa7b271c5205f304b`
- **File size:** 7430 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Manage generated regression-contract tests backed by the reconstruction store.
...
Manage generated regression-contract tests backed by the reconstruction store. Major symbols: GeneratedTests, GeneratedTests.add, GeneratedTests.synthesize, GeneratedTests.promote_counterexample, GeneratedTests.get, GeneratedTests.list, GeneratedTests.explain. Static analysis recorded 7 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: c.execute×4, destination.parent.mkdir×1, destination.write_text×1, counterexample_id.replace×1.
...
Highest structural complexity: GeneratedTests.add (branches=2, AST nodes=245), GeneratedTests.promote_counterexample (branches=2, AST nodes=153), GeneratedTests.synthesize (branches=2, AST nodes=138), GeneratedTests.list (branches=2, AST nodes=62), GeneratedTests.get (branches=1, AST nodes=68). 12 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: GeneratedTests(9), add(30), explain(9), get(100), list(167), promote_counterexample(3), synthesize(12). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: GeneratedTests(9), add(30), explain(9), get(100), list(167), promote_counterexample(3), synthesize(12). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
