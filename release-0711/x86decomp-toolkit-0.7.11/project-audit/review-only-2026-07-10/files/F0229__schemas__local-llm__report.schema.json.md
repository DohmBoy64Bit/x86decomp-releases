# Per-file audit: `schemas/local-llm/report.schema.json`

## A. File Identity

- **Inventory ID:** `F0229`
- **Relative path:** `schemas/local-llm/report.schema.json`
...
- **Apparent responsibility:** Structured repository data with top-level keys: $schema, $id, title, type, required, properties, allOf.
...
Structured repository data with top-level keys: $schema, $id, title, type, required, properties, allOf. JSON parsing was verified; top-level type is dict.
