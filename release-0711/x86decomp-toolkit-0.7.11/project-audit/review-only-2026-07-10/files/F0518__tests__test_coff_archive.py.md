# Per-file audit: `tests/test_coff_archive.py`

## A. File Identity

- **Inventory ID:** `F0518`
- **Relative path:** `tests/test_coff_archive.py`
...
- **Key imports/dependencies:** `__future__`, `pathlib`, `pytest`, `shutil`, `subprocess`, `x86decomp.coff_archive`, `x86decomp.test_bundle`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_real_coff_archive_members_symbols_and_comdat, test_static_library_is_inspected_in_safe_bundle, test_real_import_library_records. Static analysis recorded 10 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: subprocess.run×6, source.write_text×2, first_source.write_text×1, second_source.write_text×1.
...
Highest structural complexity: test_real_coff_archive_members_symbols_and_comdat (branches=8, AST nodes=304), test_real_import_library_records (branches=5, AST nodes=240), test_static_library_is_inspected_in_safe_bundle (branches=2, AST nodes=228).
