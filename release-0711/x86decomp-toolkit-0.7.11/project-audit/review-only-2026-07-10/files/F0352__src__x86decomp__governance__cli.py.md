# Per-file audit: `src/x86decomp/governance/cli.py`

## A. File Identity

- **Inventory ID:** `F0352`
- **Relative path:** `src/x86decomp/governance/cli.py`
- **SHA-256:** `c60bde3724514111ea08dbd1bedca164ea53eb7c92dbb06d2675be191b857fb9`
- **File size:** 18668 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.governance.cli` module.
...
- **Key imports/dependencies:** `.campaigns`, `.candidates`, `.changesets`, `.consensus`, `.counterexamples`, `.family`, `.hypotheses`, `.knowledge_graph`, `.plugins`, `.proofs`, `.reviews`, `.store`, `.workers`, `__future__`, `argparse`, `json`, `pathlib`, `typing`, `x86decomp.cli_utils`, `x86decomp.contracts`

## B. Functional Summary

Provide the current runtime implementation for the `x86decomp.governance.cli` module. Major symbols: build_parser, dispatch, main. No side-effect-relevant call matched the audit scanner’s rule set.
...
Highest structural complexity: dispatch (branches=65, AST nodes=1967), build_parser (branches=1, AST nodes=2000), main (branches=0, AST nodes=27), _store (branches=0, AST nodes=19). 36 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
dispatch has 65 branch/compound nodes; treat this as a profiling and maintainability target, not a confirmed performance defect.
