...
- **Inventory ID:** `F0307`
- **Relative path:** `src/x86decomp/coff_archive.py`
- **SHA-256:** `6f6e0159c3daf0dd8288d7284eff16ad341cbb4bea6f9e7e4b6afef656e475d7`
- **File size:** 16564 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Bounded Microsoft/Unix COFF archive (``.lib``/``.a``) inspection.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `src/x86decomp/linker_reconstruction.py`, `src/x86decomp/target_pack.py`, `src/x86decomp/test_bundle.py`, `tests/test_coff_archive.py`
- **Key imports/dependencies:** `.coff`, `.errors`, `.util`, `__future__`, `dataclasses`, `pathlib`, `struct`, `typing`

## B. Functional Summary

Bounded Microsoft/Unix COFF archive (``.lib``/``.a``) inspection. Major symbols: ArchiveMember, CoffArchive, parse_coff_archive_bytes, parse_coff_archive, ArchiveMember.to_dict, CoffArchive.to_dict. No side-effect-relevant call matched the audit scanner’s rule set.
...
Highest structural complexity: parse_coff_archive_bytes (branches=25, AST nodes=871), _second_linker_symbols (branches=9, AST nodes=297), _parse_import_object (branches=6, AST nodes=284), _first_linker_symbols (branches=4, AST nodes=140), _cstring_sequence (branches=3, AST nodes=124).
...
Static search found direct symbol references in tests: ArchiveMember(1), CoffArchive(1), parse_coff_archive(4), parse_coff_archive_bytes(1), to_dict(104). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: ArchiveMember(1), CoffArchive(1), parse_coff_archive(4), parse_coff_archive_bytes(1), to_dict(104). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
