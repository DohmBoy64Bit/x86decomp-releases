# Per-file audit: `src/x86decomp/reconstruction/capsules.py`

## A. File Identity

- **Inventory ID:** `F0403`
- **Relative path:** `src/x86decomp/reconstruction/capsules.py`
- **SHA-256:** `7c3d47dd5a6c1eff6597eeaaeb0fffaea295d82121cfe6c3e453b9389b4d98ec`
- **File size:** 4668 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Provide the current runtime implementation for the `x86decomp.reconstruction.capsules` module.
...
- **Key imports/dependencies:** `.store`, `__future__`, `hashlib`, `json`, `pathlib`, `typing`, `x86decomp.contracts`, `zipfile`

## B. Functional Summary

Provide the current runtime implementation for the `x86decomp.reconstruction.capsules` module. Major symbols: Capsules, Capsules.create, Capsules.inspect, Capsules.verify, Capsules.reproduce. Static analysis recorded 5 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: output.parent.mkdir×1, destination.mkdir×1, c.execute×1, target.parent.mkdir×1, target.write_bytes×1.
...
Highest structural complexity: Capsules.create (branches=6, AST nodes=444), Capsules.verify (branches=5, AST nodes=217), Capsules.reproduce (branches=3, AST nodes=164), Capsules.inspect (branches=0, AST nodes=84), Capsules.__init__ (branches=0, AST nodes=21). 9 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: Capsules(7), create(65), inspect(24), reproduce(6), verify(45). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: Capsules(7), create(65), inspect(24), reproduce(6), verify(45). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
