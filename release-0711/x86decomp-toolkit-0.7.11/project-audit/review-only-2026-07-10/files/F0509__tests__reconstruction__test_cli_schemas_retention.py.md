# Per-file audit: `tests/reconstruction/test_cli_schemas_retention.py`

## A. File Identity

- **Inventory ID:** `F0509`
- **Relative path:** `tests/reconstruction/test_cli_schemas_retention.py`
- **SHA-256:** `9608ba2363ab2e6ffe04f93add48fb747faa6cd197afad40fa20f350dbc0cb87`
...
- **Key imports/dependencies:** `__future__`, `json`, `jsonschema`, `pathlib`, `pytest`, `x86decomp.cli`, `x86decomp.cli_utils`, `x86decomp.contracts`, `x86decomp.reconstruction.cli`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_reconstruction_json_argument_validation, test_reconstruction_cli_has_complete_project_scale_surface, test_every_reconstruction_leaf_command_parses_help, test_main_cli_exposes_reconstruction_capabilities, test_all_reconstruction_schemas_are_meta_valid, test_governance_capabilities_are_in_the_unified_catalog, _leaf_commands.walk. No side-effect-relevant call matched the audit scanner’s rule set.
...
Module docstring present: yes. Documented definitions: 8/8. 2 generic low-information docstring pattern hit(s) are enumerated in evidence/generic-docstrings.json and tracked by DOC-001.

Documentation was judged proportionally: public/non-obvious behavior requires specific contracts, while trivial fixtures and data files do not require artificial prose.

## E. Code Quality and Maintainability

Highest structural complexity: _leaf_commands (branches=5, AST nodes=111), _leaf_commands.walk (branches=4, AST nodes=81), test_main_cli_exposes_reconstruction_capabilities (branches=3, AST nodes=119), test_reconstruction_cli_has_complete_project_scale_surface (branches=2, AST nodes=99), test_every_reconstruction_leaf_command_parses_help (branches=2, AST nodes=70). 1 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Executed in the exact inventory: 6 collected test(s), all passing.
...
- **Test confidence:** Executed in the exact inventory: 6 collected test(s), all passing.
