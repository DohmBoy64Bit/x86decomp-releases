# Per-file audit: `tests/test_local_llm.py`

## A. File Identity

- **Inventory ID:** `F0530`
- **Relative path:** `tests/test_local_llm.py`
- **SHA-256:** `b9b5f2bae18879a5f3d20ab4ed2e2ec5827aaa35019db764d9d39b2e287d3eaf`
- **File size:** 14111 bytes
...
- **Key imports/dependencies:** `__future__`, `contextlib`, `http.server`, `json`, `pathlib`, `pytest`, `shutil`, `threading`, `typing`, `x86decomp.coff`, `x86decomp.compiler`, `x86decomp.contracts`, `x86decomp.local_llm.matching`, `x86decomp.local_llm.profiles`, `x86decomp.local_llm.prompts`, `x86decomp.local_llm.transport`, `x86decomp.util`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_provider_profiles_and_loopback_policy, test_prompt_is_deterministic_and_marks_injected_evidence_untrusted, test_openai_generation_and_structured_output_fallback, test_ollama_probe_accepts_latest_tag_only_for_ollama, test_same_origin_redirect_rejects_cross_origin, test_exact_byte_match_loop_and_tamper_detection, test_candidate_contract_rejects_inline_assembly_and_multiple_functions, test_local_llm_helper_bodies_are_exercised, _ScriptedHandler.log_message. Static analysis recorded 2 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: good_source_path.write_text×1, mnemonic_path.write_text×1.

Boundary conditions, malformed input, error propagation, resource cleanup, path handling, encoding, integer/size limits, and partial-operation behavior were assessed where applicable. Findings are listed in Section L; where no finding is listed, the audit did not establish a defect rather than proving none exists.

## D. Documentation and Docstring Review

Module docstring present: yes. Documented definitions: 16/16. 7 generic low-information docstring pattern hit(s) are enumerated in evidence/generic-docstrings.json and tracked by DOC-001.

Documentation was judged proportionally: public/non-obvious behavior requires specific contracts, while trivial fixtures and data files do not require artificial prose.

## E. Code Quality and Maintainability

Highest structural complexity: _ScriptedHandler._respond (branches=3, AST nodes=225), _server (branches=1, AST nodes=145), test_provider_profiles_and_loopback_policy (branches=1, AST nodes=140), test_exact_byte_match_loop_and_tamper_detection (branches=0, AST nodes=338), test_local_llm_helper_bodies_are_exercised (branches=0, AST nodes=197).
