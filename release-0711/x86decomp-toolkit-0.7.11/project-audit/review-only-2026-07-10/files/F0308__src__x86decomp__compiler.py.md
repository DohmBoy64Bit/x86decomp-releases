...
- **Inventory ID:** `F0308`
- **Relative path:** `src/x86decomp/compiler.py`
- **SHA-256:** `308dd1aa876aa7a5dfea0d5760e7065cc73e71fa7a3abd2a991462a6972b06ba`
- **File size:** 9773 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Deterministic external compiler profile execution and cache keys.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `src/x86decomp/compiler_lab.py`, `src/x86decomp/compiler_worker.py`, `src/x86decomp/local_llm/matching.py`, `tests/test_compiler.py`, `tests/test_local_llm.py`
- **Key imports/dependencies:** `.errors`, `.util`, `__future__`, `os`, `pathlib`, `re`, `shutil`, `subprocess`, `tempfile`, `typing`

## B. Functional Summary

Deterministic external compiler profile execution and cache keys. Major symbols: load_profile, compiler_cache_key, run_compiler_profile. Static analysis recorded 8 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: output.parent.mkdir×2, subprocess.run×2, output.unlink×1, workdir.mkdir×1, argument.replace×1, cache_root.mkdir×1.
...
Highest structural complexity: run_compiler_profile (branches=26, AST nodes=953), load_profile (branches=25, AST nodes=403), _resolve_executable (branches=4, AST nodes=92), _tool_version (branches=3, AST nodes=93), compiler_cache_key (branches=0, AST nodes=55).
...
Static search found direct symbol references in tests: compiler_cache_key(1), load_profile(7), run_compiler_profile(5). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: compiler_cache_key(1), load_profile(7), run_compiler_profile(5). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
