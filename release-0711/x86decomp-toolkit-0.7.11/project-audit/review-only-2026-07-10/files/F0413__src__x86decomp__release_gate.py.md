# Per-file audit: `src/x86decomp/release_gate.py`

## A. File Identity

- **Inventory ID:** `F0413`
- **Relative path:** `src/x86decomp/release_gate.py`
- **SHA-256:** `e3d9af5cac0995c85f39a56e83c118f265ac6169b6d5da7f426be9abebe0e752`
- **File size:** 8060 bytes
- **Language/format:** Python source
- **Classification:** First-party source code
- **Apparent responsibility:** Target release acceptance gate.
- **Provenance:** First-party or repository-maintained text
- **Review applicability:** Complete-content text/source review
- **Related files / static consumers:** `src/x86decomp/cli.py`, `tests/test_production.py`
- **Key imports/dependencies:** `.evidence`, `.project_state`, `.reproducibility`, `.target_pack`, `.util`, `__future__`, `pathlib`, `sqlite3`, `typing`

## B. Functional Summary

Target release acceptance gate. Major symbols: evaluate_release_gate. Static analysis recorded 2 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: sqlite3.connect×1, connection.execute×1.
...
Highest structural complexity: evaluate_release_gate (branches=22, AST nodes=612), _workflow_gate (branches=7, AST nodes=340), _pipeline_gate (branches=4, AST nodes=149), _claim_gate (branches=2, AST nodes=151). 3 line(s) exceed 140 characters; this is a maintainability signal, not by itself a defect.
...
Static search found direct symbol references in tests: evaluate_release_gate(4). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
...
- **Test confidence:** Static search found direct symbol references in tests: evaluate_release_gate(4). The exact 258-test inventory passed; this is evidence of use, not a line-coverage claim.
