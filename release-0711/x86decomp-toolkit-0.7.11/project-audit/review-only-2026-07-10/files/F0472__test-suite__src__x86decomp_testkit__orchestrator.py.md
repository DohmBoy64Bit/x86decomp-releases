# Per-file audit: `test-suite/src/x86decomp_testkit/orchestrator.py`

## A. File Identity

- **Inventory ID:** `F0472`
- **Relative path:** `test-suite/src/x86decomp_testkit/orchestrator.py`
- **SHA-256:** `5faea35289fbdb17b80c9df0b31079d42608c1e6e2a15176b8835904369bdc7e`
- **File size:** 4750 bytes
- **Language/format:** Python source
- **Classification:** First-party verification-harness source
- **Apparent responsibility:** Provide the installed test-suite implementation for the `x86decomp_testkit.orchestrator` module.
...
- **Key imports/dependencies:** `.adapters`, `.adapters.capabilities`, `.config`, `.live_adapters`, `.logging_utils`, `.models`, `.reports`, `.suites`, `.timeutil`, `__future__`, `json`, `pathlib`, `uuid`

## B. Functional Summary

Provide the installed test-suite implementation for the `x86decomp_testkit.orchestrator` module. Major symbols: package_root, suite_root, feature_catalog_path, run_all. Static analysis recorded 1 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: output_directory.mkdir×1.
...
Highest structural complexity: run_all (branches=2, AST nodes=536), suite_root (branches=0, AST nodes=32), package_root (branches=0, AST nodes=17), feature_catalog_path (branches=0, AST nodes=16).
