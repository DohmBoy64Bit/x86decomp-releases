# Per-file audit: `tests/test_integration.py`

## A. File Identity

- **Inventory ID:** `F0528`
- **Relative path:** `tests/test_integration.py`
- **SHA-256:** `d67c4f725f8da2a5742c17a5ab1cc6d5395513e71eab1e382f9ff9479dbc6f6b`
...
- **Key imports/dependencies:** `__future__`, `json`, `pathlib`, `pytest`, `sys`, `x86decomp.errors`, `x86decomp.integration`

## B. Functional Summary

Executable regression tests for the behavior indicated by the filename and test symbols. Major symbols: test_integration_requires_explicit_host_consent, test_integration_equivalence_and_difference. Static analysis recorded 3 filesystem, process, database, network, or other side-effect-relevant call(s).
...
Python parsed successfully with the standard-library AST parser. Side-effect/trust-sensitive calls reviewed: path.write_text×1, tmp_path.mkdir×1, manifest.write_text×1.
...
Highest structural complexity: test_integration_equivalence_and_difference (branches=1, AST nodes=101), _manifest (branches=0, AST nodes=138), test_integration_requires_explicit_host_consent (branches=0, AST nodes=32), _write_program (branches=0, AST nodes=25).
