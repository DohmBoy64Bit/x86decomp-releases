...
from x86decomp.models import ClaimState, EvidenceKind, VerificationStatus
...
def test_model_status_enums_expose_stable_public_values() -> None:
    """Pin the public string values of the model status enumerations.

    ``VerificationStatus`` and ``ClaimState`` are part of the toolkit's public
    model surface and are serialized into evidence and verification payloads, so
    their string values are a compatibility contract. This test fixes those
    values so an accidental rename is caught as a breaking change.
    """
    assert [member.value for member in VerificationStatus] == ["not_run", "failed", "passed"]
