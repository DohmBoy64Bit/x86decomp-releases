...
def write_markdown_report(summary: RunSummary, path: Path) -> None:
    ...
    for capability in summary.capability_results:
        providers = ", ".join(getattr(capability, "providers", ()))
        diagnostics = "<br>".join(getattr(capability, "diagnostics", ()))
        capability_id = getattr(capability, "capability_id", "")
        satisfied = getattr(capability, "satisfied", False)
    ...
    for result in summary.test_results:
        ...
        joined_logs = "<br>".join(logs)
        lines.append(
            f"| {_status_icon(result.status)} | `{result.suite}` | `{result.test_id}` | "
            f"{result.duration_seconds:.3f}s | {escaped_summary} | {joined_logs} |"
        )
