...
def validate_java_syntax() -> None:
    """Validate Java syntax and report a clear message when javalang is unavailable."""
    try:
        import javalang
    except ModuleNotFoundError as exc:
        raise RuntimeError(
            "missing validation dependency: install the dev extra that provides javalang>=0.13,<1"
        ) from exc
...
def validate_static_lint() -> None:
    """Run a pyflakes undefined-name and dead-code gate over release Python sources."""
    try:
        from pyflakes.api import checkRecursive
        from pyflakes.reporter import Reporter
    except ModuleNotFoundError as exc:
        raise RuntimeError(
            "missing validation dependency: install the dev extra that provides pyflakes>=3.3,<4"
        ) from exc
    import io
    ...
    warnings = checkRecursive(
        [str(ROOT / "src"), str(ROOT / "scripts"), str(ROOT / "test-suite" / "src")],
        reporter,
    )
    details = output.getvalue().strip()
    if warnings:
        raise ValueError("pyflakes static lint failed" + (f":\n{details}" if details else ""))
...
def validate_current_release_shape() -> None:
    ...
    if project["project"]["version"] != CURRENT_VERSION or suite["project"]["version"] != CURRENT_VERSION:
        raise ValueError(f"toolkit and test-suite versions must match {CURRENT_VERSION}")
...
def main() -> None:
    ...
    validate_static_lint()
    validate_skill_frontmatter()
    validate_current_release_shape()
    print("current contracts, examples, Java syntax, static lint, schemas, skill, and release shape passed")
