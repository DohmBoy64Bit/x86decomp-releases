...
def test_worker_uses_exec_wrapper_without_thread_unsafe_preexec(
    ...
    def recording_popen(*args, **kwargs):
        """Stand in for ``subprocess.Popen``, capturing safety-relevant keyword arguments.

        Records ``preexec_fn`` and ``start_new_session`` into the enclosing ``observed`` dict,
        then delegates to the real ``Popen``.
        """
