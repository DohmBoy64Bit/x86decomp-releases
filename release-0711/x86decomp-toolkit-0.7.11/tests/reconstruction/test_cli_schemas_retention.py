...
from x86decomp.reconstruction.cli import build_parser, main as reconstruction_main
...
def test_reconstruction_json_argument_validation() -> None:
    ...
    assert parse_json_arg(None,{'default':True})=={'default':True}
    assert parse_json_arg('{"value":1}',{})=={'value':1}
    import pytest
    from x86decomp.contracts import ContractError
    with pytest.raises(ContractError): parse_json_arg('{broken',{})
