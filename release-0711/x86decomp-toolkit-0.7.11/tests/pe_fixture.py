...
def build_minimal_pe32(path: Path, code: bytes = b"\x55\x8b\xec\x31\xc0\x5d\xc3") -> Path:
    """Write a minimal but valid 32-bit PE executable to disk for tests.

    Emits a DOS stub, PE/COFF headers, a PE32 optional header, and a single executable
    ``.text`` section containing ``code``.

    Args:
        path: Destination path for the generated executable.
        code: Machine-code bytes to place in the ``.text`` section.
...
def build_minimal_pe64(path: Path, code: bytes = b"\x31\xc0\xc3") -> Path:
    """Write a minimal but valid 64-bit PE executable to disk for tests.
