...
class WorkerLimits:
    """Store the validated fields for worker limits records.
...
@dataclass(frozen=True)
class WorkerRequest:
    """Store the validated fields for worker request records.
...
@dataclass(frozen=True)
class WorkerResult:
    """Store the validated fields for worker result records.
...
def discover_worker_capabilities() -> dict[str, Any]:
    """Implement discover worker capabilities.
...
def execute_worker_request(
    ...
    *,
    ...
    def resource_wrapped_command(value: tuple[str, ...]) -> tuple[str, ...]:
        """Implement resource wrapped command.
