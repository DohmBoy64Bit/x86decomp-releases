"""Provide x86decomp.governance.cli functionality for the x86decomp-toolkit 0.7.9 release.
...
def build_parser(*, prog: str = "x86decomp-governance") -> argparse.ArgumentParser:
    ...
    Parameters and return values follow the signature and runtime validation in the body.
    """
    parser = argparse.ArgumentParser(prog=prog, description="x86decomp-toolkit 0.7.9 evidence-driven convergence control plane")
...
def dispatch(args: argparse.Namespace) -> Any:
    ...
    if args.group == "campaign":
        ...
        if args.action=="create": return api.create(args.goal,budget=_json_arg(args.budget_json,{}),policy=_json_arg(args.policy_json,{}),actor=actor)
    ...
    if args.group == "hypothesis":
        ...
        if args.action=="evidence": return api.attach_evidence(args.hypothesis_id,args.evidence_id,stance=args.stance,weight=args.weight,evidence_kind=args.kind,independence_group=args.group,artifact_sha256=args.artifact_sha256,details=_json_arg(args.details_json,{}),actor=actor)
    ...
    if args.group == "candidate":
        ...
        if args.action=="create": cid=api.create(args.branch_name,campaign_id=args.campaign_id,parent_candidate_id=args.parent,objective=_json_arg(args.objective_json,{}),actor=actor); return api.get(cid)
        if args.action=="add-file": return api.add_file(args.candidate_id,args.source,args.relative_path,actor=actor)
        if args.action=="evaluate": eid=api.record_evaluation(args.candidate_id,args.metric,args.status,value=args.value,details=_json_arg(args.details_json,{}),actor=actor); return {"evaluation_id":eid,"candidate":api.get(args.candidate_id)}
    ...
    if args.group == "counterexample":
        ...
        if args.action=="add": cid=api.add(args.scope_kind,args.scope_id,args.payload,predicate=_json_arg(args.predicate_json,{}),provenance=_json_arg(args.provenance_json,{}),actor=actor); return api.get(cid)
    ...
    if args.group == "consensus":
        ...
        if args.action=="resolve": rid=api.resolve(args.subject_kind,args.subject_id,args.property_name,_json_arg(args.selected_value_json,None),method=args.method,rationale=args.rationale,actor=actor,lock=args.lock); return {"resolution_id":rid}
        return api.explain(args.subject_kind,args.subject_id,args.property_name)
    if args.group == "graph":
        api=KnowledgeGraph(store)
        if args.action=="node": return api.upsert_node(args.node_id,args.kind,args.label,attributes=_json_arg(args.attributes_json,{}),actor=actor)
        if args.action=="edge": return {"edge_id":api.add_edge(args.source_id,args.target_id,args.relation,attributes=_json_arg(args.attributes_json,{}),actor=actor)}
    ...
    if args.group == "review":
        ...
        if args.action=="create": rid=api.create(args.kind,args.subject_id,args.summary,priority=args.priority,details=_json_arg(args.details_json,{}),actor=actor); return api.get(rid)
    ...
    if args.group == "family":
        ...
        if args.action=="add": return api.add(args.family_id,args.label,args.path,metadata=_json_arg(args.metadata_json,{}),actor=actor)
    ...
    if args.group == "proof":
        ...
        if args.action=="obligation": return ledger.create_obligation(args.scope_kind,args.scope_id,args.property_name,args.required_status,assumptions=_json_arg(args.assumptions_json,[]),actor=actor)
        if args.action=="result": return ledger.add_result(args.obligation_id,args.status,args.validator,_json_arg(args.report_json,{}),artifact_sha256=args.artifact_sha256,actor=actor)
    ...
    if args.group == "worker":
        ...
        if args.action=="register": return api.register(args.name,_json_arg(args.capabilities_json,{}),endpoint=args.endpoint,environment_sha256=args.environment_sha256,actor=actor)
        ...
        return api.select(_json_arg(args.required_json,{}))
    if args.group == "plugin":
        ...
        return api.invoke(args.plugin_id,args.capability,_json_arg(args.request_json,{}),timeout_seconds=args.timeout,actor=actor)
