...
class TypeDescriptorRecord:
    """Store the validated fields for type descriptor record records.
...
class BaseClassRecord:
    """Store the validated fields for base class record records.
...
class CompleteObjectLocatorRecord:
    """Store the validated fields for complete object locator record records.
...
@dataclass(frozen=True)
class VTableRecord:
    """Store the validated fields for v table record records.
...
class UnwindCodeRecord:
    """Store the validated fields for unwind code record records.
...
class UnwindInfoRecord:
    """Store the validated fields for unwind info record records.
...
class PEView:
    """Represent p e view state and behavior.
    ...
    def encoded_pointer_to_rva(self, value: int) -> int | None:
        """Implement encoded pointer to rva.
    ...
    def readonly_data_sections(self) -> tuple[Section, ...]:
        """Implement readonly data sections.
