...
class ArchiveMember:
    """Store the validated fields for archive member records.
...
@dataclass(frozen=True)
class CoffArchive:
    """Store the validated fields for coff archive records.
...
def _first_linker_symbols(data: bytes) -> list[tuple[str, int]] | None:
    """Implement first linker symbols.
...
def _second_linker_symbols(data: bytes) -> list[tuple[str, int]] | None:
    """Implement second linker symbols.
