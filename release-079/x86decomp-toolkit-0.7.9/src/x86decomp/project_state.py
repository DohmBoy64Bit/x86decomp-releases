...
from .util import atomic_write_bytes, canonical_json_bytes, load_json, sha256_bytes, sha256_file, utc_now, write_json
...
class ProjectCheck:
    """Store the validated fields for project check records.
...
class ProjectStateDatabase:
    """Represent project state database state and behavior.
    
    Attributes, invariants, and helper methods are defined by the class body.
    """
    def __init__(self, path: Path, *, toolkit_version: str = "0.7.9"):
    ...
    def upsert_artifact_reference(self, reference: str, digest: str, kind: str, owner: str) -> None:
        """Insert or update artifact reference.
...
def migrate_project(
    ...
    *,
    ...
    if current == 1:
        ...
        plan.append("1->2")
    working = _migration_2_to_3(root, working) if not dry_run else {**working, "schema_version": 3, "toolkit_release": "0.7.9"}
    plan.append("2->3")
    if dry_run:
