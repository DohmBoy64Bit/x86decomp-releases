"""Provide tests.test_documentation functionality for the x86decomp-toolkit 0.7.9 release.
...
def test_release_documents_and_maps_are_current() -> None:
    """Verify release documents and maps are current.
    ...
    for path in required:
        ...
        assert "0.7.9" in text, path


def test_test_plan_counts_match_feature_catalog() -> None:
    """Verify test plan counts match feature catalog.
...
def test_four_architecture_artifacts_exist_and_cross_link() -> None:
    """Verify four architecture artifacts exist and cross link.
    ...
    for path in (toolkit_mermaid, toolkit_ascii, suite_mermaid, suite_ascii):
        assert path.is_file()
        assert "0.7.9" in path.read_text(encoding="utf-8")
