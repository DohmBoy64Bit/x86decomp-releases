"""Provide tests.reconstruction.test_project_headers_builds functionality for the x86decomp-toolkit 0.7.9 release.
...
def test_store_is_additive_and_layout_is_evidence_labeled(tmp_path: Path) -> None:
    """Verify store is additive and layout is evidence labeled.
...
def test_headers_reject_cycles_and_generate_compileable_shape(tmp_path: Path) -> None:
    """Verify headers reject cycles and generate compileable shape.
...
def test_historical_and_portable_build_modes_remain_distinct(tmp_path: Path) -> None:
    """Verify historical and portable build modes remain distinct.
