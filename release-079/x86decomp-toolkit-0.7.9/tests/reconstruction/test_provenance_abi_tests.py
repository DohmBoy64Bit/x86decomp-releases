"""Provide tests.reconstruction.test_provenance_abi_tests functionality for the x86decomp-toolkit 0.7.9 release.
...
from x86decomp.governance.proofs import ProofLedger
...
def test_provenance_round_trip_edit_reconciliation_and_locks(tmp_path: Path) -> None:
    """Verify provenance round trip edit reconciliation and locks.
...
def test_abi_contract_verification_comparison_and_explicit_shim(tmp_path: Path) -> None:
    """Verify abi contract verification comparison and explicit shim.
...
def test_generated_tests_are_evidence_bound_and_counterexamples_promote(tmp_path: Path) -> None:
    """Verify generated tests are evidence bound and counterexamples promote.
