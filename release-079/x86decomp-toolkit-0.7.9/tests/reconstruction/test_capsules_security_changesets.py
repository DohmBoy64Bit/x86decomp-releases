"""Provide tests.reconstruction.test_capsules_security_changesets functionality for the x86decomp-toolkit 0.7.9 release.
...
def test_capsule_is_deterministic_verifiable_and_reproducible(tmp_path: Path) -> None:
    """Verify capsule is deterministic verifiable and reproducible.
    ...
    inspected=api.inspect(capsule['archive_path']); assert inspected['manifest']['toolkit_version']=='0.7.9'
...
def test_library_recognition_never_silently_replaces_code(tmp_path: Path) -> None:
    """Verify library recognition never silently replaces code.
...
def test_security_scan_reports_without_modifying_behavior(tmp_path: Path) -> None:
    """Verify security scan reports without modifying behavior.
...
def test_semantic_changesets_detect_conflicts_and_verify_clean_merges(tmp_path: Path) -> None:
    """Verify semantic changesets detect conflicts and verify clean merges.
