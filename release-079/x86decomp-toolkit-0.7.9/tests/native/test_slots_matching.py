"""Provide tests.native.test_slots_matching functionality for the x86decomp-toolkit 0.7.9 release.
...
def test_slot_audit_classifies_exact_padding_and_overlap(tmp_path:Path)->None:
    """Verify slot audit classifies exact padding and overlap.
...
def test_padding_comparison_is_tail_only_and_protected_aware()->None:
    """Verify padding comparison is tail only and protected aware.
...
def test_batch_matching_records_safe_and_fallback(tmp_path:Path)->None:
    """Verify batch matching records safe and fallback.
...
def test_short_candidate_requires_padding_only_remainder(tmp_path:Path)->None:
    """Verify short candidate requires padding only remainder.
