"""Provide tests.native.test_pe_hybrid functionality for the x86decomp-toolkit 0.7.9 release.
...
def test_safe_patch_and_text_swap_preserve_pe_container(tmp_path:Path)->None:
    """Verify safe patch and text swap preserve pe container.
...
def test_section_export_and_synthetic_coff_for_pe32_and_pe64(tmp_path:Path)->None:
    """Verify section export and synthetic coff for pe32 and pe64.
...
def test_hybrid_composer_promotes_only_verified_candidates(tmp_path:Path)->None:
    """Verify hybrid composer promotes only verified candidates.
...
def test_hybrid_composer_rejects_candidate_changed_after_match(tmp_path:Path)->None:
    """Verify hybrid composer rejects candidate changed after match.
