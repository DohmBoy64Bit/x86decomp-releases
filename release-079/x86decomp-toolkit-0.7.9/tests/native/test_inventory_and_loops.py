"""Provide tests.native.test_inventory_and_loops functionality for the x86decomp-toolkit 0.7.9 release.
...
def test_native_cli_json_helpers(tmp_path: Path) -> None:
    """Verify native cli json helpers.
...
def test_project_inventory_audit_pe_inventory_reports_and_doctor(tmp_path: Path) -> None:
    """Verify project inventory audit pe inventory reports and doctor.
...
def test_native_list_and_verify_paths(tmp_path: Path) -> None:
    """Verify native list and verify paths.
