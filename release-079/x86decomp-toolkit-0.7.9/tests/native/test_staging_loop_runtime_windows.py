"""Provide tests.native.test_staging_loop_runtime_windows functionality for the x86decomp-toolkit 0.7.9 release.
...
def test_staging_context_resolution_and_compile_check(tmp_path:Path)->None:
    """Verify staging context resolution and compile check.
...
def test_closed_loop_requires_consent_and_records_verified_result(tmp_path:Path)->None:
    """Verify closed loop requires consent and records verified result.
...
def test_runtime_static_validation_and_crash_mapping(tmp_path:Path)->None:
    """Verify runtime static validation and crash mapping.
...
def test_windows_launcher_prefers_batch_on_windows_and_response_files(tmp_path:Path)->None:
    """Verify windows launcher prefers batch on windows and response files.
