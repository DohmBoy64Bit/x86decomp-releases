"""Provide tests.governance.test_governance_cli_schemas functionality for the x86decomp-toolkit 0.7.9 release.
...
def test_every_governance_schema_meta_validates() -> None:
    """Verify every governance schema meta validates.
...
def test_governance_cli_help_and_init(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Verify governance cli help and init.
...
def test_governance_cli_hypothesis_flow(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Verify governance cli hypothesis flow.
...
def test_contract_helpers_and_cli_json_arg(tmp_path: Path) -> None:
    """Verify contract helpers and cli json arg.
    
    Parameters and return values follow the signature and runtime validation in the body.
    """
    from x86decomp.governance.cli import _json_arg
...
def test_all_governance_leaf_commands_parse_help() -> None:
    """Verify all governance leaf commands parse help.
