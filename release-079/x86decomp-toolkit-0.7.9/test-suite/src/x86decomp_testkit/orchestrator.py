"""Provide test-suite.x86decomp_testkit.orchestrator functionality for the x86decomp-toolkit 0.7.9 release.
...
from .adapters.capabilities import capabilities_from_adapters, write_capability_report
...
def feature_catalog_path() -> Path:
    """Implement feature catalog path.
...
def run_all(config: TestConfig, *, config_path: Path | None = None, verbose: bool = False) -> RunSummary:
    ...
    capability_results = capabilities_from_adapters(catalog, adapter_results)
    write_capability_report(capability_results, output_directory / "capabilities.json")
    tests = []
