...
REQUIRED_RELEASE_ARTIFACTS = [
    ...
    "release-artifacts/DOCSITE_SYNC_AUDIT.json",
    "release-artifacts/HARNESS_SUMMARY.json",
    "release-artifacts/FULL_DOCSITE_HASH_PARITY_AUDIT.md",
    "release-artifacts/FULL_DOCSITE_HASH_PARITY_AUDIT.json",
    ...
    if not search_paths:
        ...
        for page in markdown_pages:
            ...
            if page.relative_to(DOCS).as_posix() == "index.md":
                alternates.update({"", "#x86decomp-075-documentation"})
...
def main() -> int:
    ...
    for path in root_text_files + docs_text_files:
        ...
        if re.search(r"(?<![0-9A-Fa-f])0\.7\.[0-7](?![0-9A-Fa-f])", text):
            stale.append(path.relative_to(ROOT).as_posix())
    if stale:
        errors.append("stale pre-0.7.8 references in current site source: " + ", ".join(stale[:20]))
    ...
    print("Dracula-styled 0.7.8 docs, release artifacts, built pages, search index, and local links verified.")
    return 0
