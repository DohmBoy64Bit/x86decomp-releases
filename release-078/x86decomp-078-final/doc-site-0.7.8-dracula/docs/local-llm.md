---
title: Local LLM C generation
description: Bounded local-model C proposals with deterministic exact-byte acceptance.
---
...
## 0.7.8 acceleration additions

Version 0.7.8 closes the manual job-file gap and adds batch and promotion helpers. The new local-model routes are:
...
`job-create` materializes a local-LLM job from a verified function packet. `job-from-range` creates one from an explicit byte span and records that no PE section mapping was inferred. `batch-create` emits jobs for eligible contiguous function packets and reports blocked reasons for ineligible functions. `batch-match` runs deterministic single-worker job queues and writes per-job reports. `cpp-generate` is a bounded proposal command for C++ source candidates; it remains proposal-only and does not weaken the compile/match acceptance gates.

## 0.7.8 adapter capability mapping

The test harness now separates product adapter identities from protocol capabilities. LM Studio's local server can be configured through the `lm-studio-http` endpoint adapter and can satisfy `openai-compatible-chat` and `local-loopback-llm` coverage after a loopback `/v1/models` probe. This does not mark `ollama`, `llama-server`, `localai`, or `vllm` as installed; those remain distinct product adapters and still report `BLOCKED` when unavailable.

Use one of these endpoint settings before running the harness:

```powershell
$env:LM_STUDIO_BASE_URL = "http://127.0.0.1:1234/v1"
# or
$env:X86DECOMP_LM_STUDIO_URL = "http://127.0.0.1:1234/v1"
```

The companion command shows the capability view:

```powershell
x86decomp-test capabilities --config x86decomp-test.json
```
